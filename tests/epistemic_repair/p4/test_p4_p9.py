...
def _report():
    return _seal({"hard_gates": {key: True for key in ("HG-10", "HG-11", "HG-12", "HG-13")},
...
def test_p4_sidecar_rejects_missing_raw_rows_and_extra_metrics(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="source artifact digest"):
        build_p4_p9_sidecar(report_path=_write(tmp_path, report), commit="a" * 40, worktree_clean=True)
    report = _report(); report["continuous_metrics"]["invented"] = 1; report = _seal(report)
