...
- **FR-010**: The state token in the OAuth redirect MUST be an HMAC-signed payload containing at minimum `tenant_id` (taken from the authenticated session, never from a query parameter), a `nonce`, and an `expiry_ts`; the HMAC key MUST be a server-only secret. The `nonce` MUST be recorded server-side in a tenant-scoped `oauth_install_states` table at issuance time so that single-use can be enforced; the callback MUST reject any state token whose nonce is missing, already consumed (`consumed_at IS NOT NULL`), or expired. A background sweep (or read-time TTL filter) reclaims rows older than the configured TTL.
