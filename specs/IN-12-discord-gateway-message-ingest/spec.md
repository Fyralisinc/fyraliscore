...
- Q: Trust tier value for the new `discord:message` source_channel → A: **`attested_human`** — match Slack's `slack:message` semantic. A regular Discord user posting in a channel is asserting identity via Discord's auth system, which is human-attested conversational signal. `attested_agent` is reserved for explicit bot-directed invocations like `/fyralis ask` (IN-09) where the user is talking *to* the agent rather than *with* peers. Picking the wrong tier would corrupt how downstream consumers (retrieval, scoring, audit) weight this signal — `attested_human` is the right slot.
...
**Independent Test**: With the worker connected to gateway.discord.gg, post a message in a channel of a guild that has a `provider_installations` row. Within 5 seconds assert: exactly one new row in `observations` with `tenant_id` matching the seeded tenant, `source_channel='discord:message'`, `external_id='discord:<message_id>'`, `source_actor_ref='discord:<author_id>'`, `content_text` exactly equal to the posted message body, `trust_tier='attested_human'`.
...
- **FR-009**: `services/ingestion/handlers/__init__.py::CHANNEL_TRUST_MAP` MUST be extended with `"discord:message": "attested_human"` so observations from this path carry the right `trust_tier`.
...
- **Discord Message Observation**: A row in `observations` with `source_channel='discord:message'`, `external_id='discord:<message_id>'`, `source_actor_ref='discord:<author_id>'`, `content_text=<message.content verbatim>`, `trust_tier='attested_human'`, `kind='signal'`, tenant-scoped via the existing IN-09 install row.
...
- **Trust tier `attested_human` is the right semantic value for `discord:message`**: A regular Discord user posting in a channel is asserting identity via Discord's auth system, which is comparable to Slack's `slack:message`. Confirmed during the clarify phase.
