#!/usr/bin/env python3
"""Run the sanitized BYOC customer handoff readiness report."""
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Sanitized BYOC evidence package to validate for handoff.",
    )
    parser.add_argument(
        "--evidence-ledger",
        type=Path,
        default=Path("deploy/byoc/evidence-ledger.example.yaml"),
        help="Sanitized BYOC evidence ledger referenced by the package digests.",
    )
    parser.add_argument(
        "--env-file",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="Optional path to write the sanitized customer handoff report.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_byoc_customer_handoff(
        ByocCustomerHandoffInputs(
    ...
    rendered = (
        render_customer_handoff_json(report)
        if args.json
        else render_customer_handoff_yaml(report)
    )
    if args.output is not None:
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_text(rendered, encoding="utf-8")
    sys.stdout.write(rendered)
    return 0 if report.customer_handoff_ready else 1
