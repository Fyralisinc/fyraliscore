#!/usr/bin/env python3
"""scripts/sandbox_brex.py — local end-to-end sandbox for Brex ingestion
(finance), with NO real Brex credentials.

Brex is a banking REST API (Bearer API token) with BOTH a historical query
...
Run:
    python scripts/sandbox_brex.py
    python scripts/sandbox_brex.py --keep
"""
...
_BASE_URL = "https://api.brex.com/api/v1"
_ORG_ID = "org-sandbox"
...
async def _drain_shard(pool, install_row, shard_identifier) -> list[str]:
    ...
    from services.ingest.ingestion.fetchers.brex import fetch_page_brex
    ...
    while True:
        ...
        result = await fetch_page_brex(install_row, shard_identifier, cursor)
        for record in result.records:
            res = await ingest("brex:transaction", record, pool=pool, tenant_id=_TENANT_ID)
            if not res.deduped:
...
async def run(args) -> int:
    ...
    if provided_url:
        ...
        created_db = f"brex_sandbox_{uuid4().hex[:8]}"
    ...
    try:
        ...
        await pool.execute(
            "INSERT INTO tenants (id, name) VALUES ($1, 'brex-sandbox') "
        ...
        _hr("ENUMERATE ACCOUNTS (BrexClient.list_accounts)")
        from services.ingest.ingestion.fetchers._clients import build_brex_client
        ...
        client = await build_brex_client(_Inst())
        ...
        _hr("PROVISION (brex.onboarding.finalize_install)")
        ...
        acct_count = await pool.fetchval(
            "SELECT count(*) FROM brex_accounts WHERE brex_installation_id=$1", install_id,
        )
        ...
        _check("onboarding trigger emitted (source=brex)",
               trig is not None and trig["source"] == "brex")
        ...
        counts = await pool.fetchrow(
            ...
            "FROM observations WHERE tenant_id=$1 AND source_channel='brex:transaction'",
            _TENANT_ID,
        )
        ...
        incr_shard = {"shard_kind": "brex_account_txns", "account_id": _ACCOUNT,
                      "txn_cursor": hw}
        ...
        res = await ingest("brex:transaction", twin, pool=pool, tenant_id=_TENANT_ID)
        ...
        res = await ingest("brex:transaction", webhook_payload, pool=pool, tenant_id=_TENANT_ID)
        ...
        res = await ingest("brex:transaction", fresh_txn, pool=pool, tenant_id=_TENANT_ID)
        ...
        _check("all observations are authoritative brex:transaction",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Brex ingestion sandbox")
