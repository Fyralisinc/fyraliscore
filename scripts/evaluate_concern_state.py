#!/usr/bin/env python3
"""Measure one tenant/time-bounded attention-governance/Concern population."""
...
async def _run(args: argparse.Namespace) -> int:
    ...
    try:
        state = await evaluate_concern_state(
            conn,
            scope=scope,
    ...
    manifest = InvariantEvidenceManifest(
        manifest_version="governed-concern-evidence-v1",
        run_id=args.run_id,
    ...
    if args.format == "json":
        print(
            json.dumps(
                {
                    "concern_state": state.model_dump(mode="json"),
        ...
        print(render_concern_markdown(state))
    ...
    if args.require_protocol_closure and (
        state.binding_contract_validity_rate < 1.0
        or state.snapshot_contract_validity_rate < 1.0
        or state.reducer_conformance_rate < 1.0
        or state.contributor_monotonicity_rate < 1.0
        or state.binding_envelope_conformance_rate < 1.0
        or state.disposition_capability_conformance_rate < 1.0
        or state.transition_conformance_rate < 1.0
        ...
        or state.identity_correction_reciprocity_rate < 1.0
    ):
        return 3
    return 0
