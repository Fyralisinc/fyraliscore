#!/usr/bin/env python3
"""scripts/sandbox_deel.py — local end-to-end sandbox for Deel ingestion
(finance), with NO real Deel credentials.

Deel is a contractor-payments REST API (Bearer API token) with BOTH a historical
query surface (GET /contracts, /contract/{id}/payments) and a live push surface
...
It exercises: contract enumeration, per-contract backfill with the
contract-snapshot + payment fan-out, the incremental delta (a payment status
change -> state_change), the live-webhook path through the SAME handler
...
Run:
    python scripts/sandbox_deel.py
    python scripts/sandbox_deel.py --keep
"""
...
_BASE_URL = "https://api.letsdeel.com"
_ORG_ID = "org-sandbox"
...
def _build_fixtures() -> dict:
    ...
    def payment(pid, amount, counterparty, status, created, kind="externalTransfer"):
        return {
            "id": pid,
            "amount": amount,
    ...
    return {
        _CONTRACT: {
            "contract": {
                "id": _CONTRACT,
                "name": "Eng Monthly Agreement",
                "type": "ongoing_time_based",
                "status": "in_progress",
                "rate": 8500.00,
            },
            "payments": [
                payment("p-1001", -5000.00, "Acme Cloud", "sent",
                        _iso(now - timedelta(days=3))),
                payment("p-1002", 120000.00, "Stripe Payout", "sent",
                        _iso(now - timedelta(days=2)), kind="incomingPayment"),
            ],
            # Incremental delta: p-1001 FAILS (sent -> failed) — a fresh
            # cash-risk state_change the poll/reconcile surfaces.
            "delta": [
                payment("p-1001", -5000.00, "Acme Cloud", "failed",
...
async def _drain_shard(pool, install_row, shard_identifier) -> list[str]:
    ...
    from services.ingest.ingestion.fetchers.deel import fetch_page_deel
    ...
    while True:
        ...
        result = await fetch_page_deel(install_row, shard_identifier, cursor)
        for record in result.records:
            res = await ingest("deel:payment", record, pool=pool, tenant_id=_TENANT_ID)
            if not res.deduped:
...
async def run(args) -> int:
    ...
    if provided_url:
        ...
        created_db = f"deel_sandbox_{uuid4().hex[:8]}"
    ...
    try:
        ...
        await pool.execute(
            "INSERT INTO tenants (id, name) VALUES ($1, 'deel-sandbox') "
            "ON CONFLICT (id) DO NOTHING", _TENANT_ID,
        )
        print("  Migrations applied, partitions ensured, tenant seeded.")

        # 2. Enumerate contracts via the REAL client.
        _hr("ENUMERATE CONTRACTS (DeelClient.list_contracts)")
        from services.ingest.ingestion.fetchers._clients import build_deel_client
        ...
        client = await build_deel_client(_Inst())
        contracts = await client.list_contracts()
        contract_ids = [c["id"] for c in contracts]
        print(f"  contracts discovered: {contract_ids}")
        _check("contract enumeration returned the eng contract", _CONTRACT in contract_ids)

        # 3. Provision the install + webhook row.
        _hr("PROVISION (deel.onboarding.finalize_install)")
        ...
        install_id = await finalize_install(
            ...
            contracts=[{"contract_id": c["id"], "contract_name": c.get("name"),
                        "contract_type": c.get("type")} for c in contracts],
            organization_id=_ORG_ID,
        )
        await register_webhook_installation(
            pool, tenant_id=_TENANT_ID, organization_id=_ORG_ID, webhook_secret_ref=None,
        )
        con_count = await pool.fetchval(
            "SELECT count(*) FROM deel_contracts WHERE deel_installation_id=$1", install_id,
        )
        trig = await pool.fetchrow(
            "SELECT source FROM onboarding_triggers WHERE tenant_id=$1", _TENANT_ID,
        )
        _check("install + contract rows provisioned", con_count == len(contract_ids))
        _check("onboarding trigger emitted (source=deel)",
               trig is not None and trig["source"] == "deel")
        ...
        print(f"  planned {len(shards)} shard(s): "
              + ", ".join(s.shard_identifier["contract_id"] for s in shards))
        _check("one shard per contract", len(shards) == len(contract_ids))

        # 5. Backfill: real fetcher -> real ingest.
        _hr("BACKFILL (snapshot + payment fan-out -> ingest)")
        for shard in shards:
            ext = await _drain_shard(pool, install_row, shard.shard_identifier)
            print(f"  {shard.shard_identifier['contract_id']}: ingested {len(ext)} observations")
        counts = await pool.fetchrow(
            "SELECT count(*) FILTER (WHERE kind='signal') AS sig, "
            "count(*) FILTER (WHERE kind='state_change') AS sc, count(*) AS tot "
            "FROM observations WHERE tenant_id=$1 AND source_channel='deel:payment'",
            _TENANT_ID,
        )
        print(f"  observations: total={counts['tot']} signal={counts['sig']} state_change={counts['sc']}")
        # 1 contract snapshot + 2 payments (both sent -> signal) = 3.
        _check("backfill produced 3 observations (1 snapshot + 2 payments)",
               counts["tot"] == 3)

        # 6. Incremental: warm-start from the high-water -> delta (p-1001 fails).
        _hr("INCREMENTAL (status transition: sent -> failed)")
        hw = await pool.fetchval(
            "SELECT max(content->>'created_at') FROM observations "
            "WHERE tenant_id=$1 AND content->>'object_type'='payment'", _TENANT_ID,
        )
        incr_shard = {"shard_kind": "deel_contract_payments", "contract_id": _CONTRACT,
                      "payment_cursor": hw}
        ...
        # 7. Dedup: re-ingest a backfilled payment twin -> deduped.
        _hr("DEDUP (backfill vs re-fetch twin)")
        from services.ingest.ingestion.core import ingest
        twin = {"_fyralis_record_type": "payment", "_fyralis_contract_id": _CONTRACT,
                "payment": fixtures[_CONTRACT]["payments"][1]}
        res = await ingest("deel:payment", twin, pool=pool, tenant_id=_TENANT_ID)
        _check("re-ingesting an existing payment dedups (versioned external_id parity)",
               res.deduped is True)

        # 8. LIVE WEBHOOK path: a payment.created with the SAME payment flows
        #    through the SAME handler; its external_id matches the backfilled
        #    payment, so it dedups (proves backfill+live parity).
        ...
        res = await ingest("deel:payment", webhook_payload, pool=pool, tenant_id=_TENANT_ID)
        _check("live webhook payment dedups against backfilled twin (external_id parity)",
               res.deduped is True)

        # A brand-new live payment lands as a fresh observation.
        fresh_payment = {
            "type": "payment.created", "organizationId": _ORG_ID,
            "_fyralis_contract_id": _CONTRACT,
            "payment": {"id": "p-live-1", "amount": -250.00,
                        "counterpartyName": "AWS", "status": "sent",
                        "kind": "externalTransfer",
                        "createdAt": _iso(datetime.now(timezone.utc))},
        }
        res = await ingest("deel:payment", fresh_payment, pool=pool, tenant_id=_TENANT_ID)
        _check("new live payment lands as a fresh observation", res.deduped is False)

        # 9. Reconciler gap probe against the live (mock) contract.
        _hr("RECONCILER GAP PROBE (list_payments since high-water)")
        payments, _, _ = await client.list_payments(_CONTRACT, limit=1, offset=0,
                                                     start=(hw or "2020-01-01")[:10])
        _check("reconciler probe detects a payment since the high-water", len(payments) >= 1)
        ...
        _check("all observations are authoritative deel:payment",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Deel ingestion sandbox")
