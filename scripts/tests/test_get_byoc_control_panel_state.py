from __future__ import annotations

import json

from scripts.get_byoc_control_panel_state import (
    CONTROL_PANEL_STATE_PATH,
    DEFAULT_SIGNING_SECRET_ENV,
    main,
)
from services.platform.runtime.byoc_control_plane_intake import (
    validate_evidence_receipt_read_auth_headers,
)


SIGNING_SECRET = "local-control-panel-state-read-secret"
SIGNING_KEY_REF = "control-plane/byoc/evidence-read-key"
DEPLOYMENT_ID = "dep_panelcli01"
CUSTOMER_ID = "cus_panelcli01"


def _base_args() -> list[str]:
    return [
        "--deployment-id",
        DEPLOYMENT_ID,
        "--customer-id",
        CUSTOMER_ID,
        "--recent-limit",
        "7",
        "--key-ref",
        SIGNING_KEY_REF,
        "--nonce",
        "nonce-control-panel-state-test-001",
        "--timestamp",
        "2026-06-27T12:00:00+00:00",
    ]


def test_get_byoc_control_panel_state_prints_signed_get_request(
    monkeypatch,
    capsys,
) -> None:
    monkeypatch.setenv(DEFAULT_SIGNING_SECRET_ENV, SIGNING_SECRET)

    code = main(_base_args())

    captured = capsys.readouterr()
    payload = json.loads(captured.out)
    rendered = json.dumps(payload, sort_keys=True)
    assert code == 0
    assert payload["method"] == "GET"
    assert payload["path"] == CONTROL_PANEL_STATE_PATH
    assert payload["query"] == (
        f"deployment_id={DEPLOYMENT_ID}&customer_id={CUSTOMER_ID}&recent_limit=7"
    )
    assert validate_evidence_receipt_read_auth_headers(
        payload["headers"],
        method="GET",
        path=payload["path"],
        query=payload["query"],
        signing_secret=SIGNING_SECRET,
        expected_key_ref=SIGNING_KEY_REF,
        max_clock_skew_seconds=10**9,
    ) == []
    assert SIGNING_SECRET not in rendered
    assert "install_token" not in rendered.lower()
    assert "secret_ref" not in rendered.lower()
    assert "payload" not in rendered.lower()
    assert captured.err == ""


def test_get_byoc_control_panel_state_executes_signed_read(
    monkeypatch,
    capsys,
) -> None:
    posted: dict[str, object] = {}
    monkeypatch.setenv(DEFAULT_SIGNING_SECRET_ENV, SIGNING_SECRET)

    class _Response:
        def __enter__(self):
            return self

        def __exit__(self, exc_type, exc, tb) -> bool:
            return False

        def read(self) -> bytes:
            return json.dumps(
                {
                    "schema_version": "fyralis.byoc.control_panel_state.v1",
                    "deployment_id": DEPLOYMENT_ID,
                    "customer_id": CUSTOMER_ID,
                    "generated_at": "2026-06-27T12:00:00Z",
                    "overview": {"status": "ready"},
                    "sections": [
                        {
                            "key": "deployment_overview",
                            "status": "ready",
                            "item_count": 1,
                            "source_schema_version": (
                                "fyralis.byoc.deployment_overview.v1"
                            ),
                        }
                    ],
                    "actions": [],
                    "agent_fleet": {"result_count": 1, "items": []},
                    "evidence_packages": {"result_count": 1, "items": []},
                    "preflight_reports": {"result_count": 1, "items": []},
                    "runner_evidence": {"result_count": 1, "items": []},
                    "stored_scope": "sanitized_control_panel_metadata_only",
                }
            ).encode("utf-8")

    def _fake_urlopen(request, *, timeout):
        posted["url"] = request.full_url
        posted["timeout"] = timeout
        posted["headers"] = dict(request.headers)
        return _Response()

    monkeypatch.setattr(
        "scripts.get_byoc_control_panel_state.urllib.request.urlopen",
        _fake_urlopen,
    )

    code = main(
        [
            *_base_args(),
            "--control-panel-state-url",
            "https://control.example.com/byoc/control-plane/control-panel-state",
            "--timeout-seconds",
            "3",
        ]
    )

    captured = capsys.readouterr()
    state = json.loads(captured.out)
    rendered_headers = json.dumps(posted["headers"], sort_keys=True)
    assert code == 0
    assert posted["url"] == (
        "https://control.example.com/byoc/control-plane/control-panel-state"
        f"?deployment_id={DEPLOYMENT_ID}&customer_id={CUSTOMER_ID}&recent_limit=7"
    )
    assert posted["timeout"] == 3.0
    assert state["schema_version"] == "fyralis.byoc.control_panel_state.v1"
    assert state["stored_scope"] == "sanitized_control_panel_metadata_only"
    assert state["sections"][0]["key"] == "deployment_overview"
    assert SIGNING_SECRET not in rendered_headers
    assert "install_token" not in captured.out.lower()
    assert "secret_ref" not in captured.out.lower()
    assert "payload" not in captured.out.lower()


def test_get_byoc_control_panel_state_requires_signing_secret(capsys) -> None:
    code = main(_base_args())

    captured = capsys.readouterr()
    assert code == 2
    assert DEFAULT_SIGNING_SECRET_ENV in captured.err
    assert captured.out == ""


def test_get_byoc_control_panel_state_requires_key_ref(monkeypatch, capsys) -> None:
    monkeypatch.setenv(DEFAULT_SIGNING_SECRET_ENV, SIGNING_SECRET)
    args = _base_args()
    key_ref_index = args.index("--key-ref")
    del args[key_ref_index : key_ref_index + 2]

    code = main(args)

    captured = capsys.readouterr()
    assert code == 2
    assert "--key-ref is required" in captured.err
    assert captured.out == ""


def test_get_byoc_control_panel_state_requires_deployment_id(
    monkeypatch,
    capsys,
) -> None:
    monkeypatch.setenv(DEFAULT_SIGNING_SECRET_ENV, SIGNING_SECRET)
    args = _base_args()
    deployment_id_index = args.index("--deployment-id")
    del args[deployment_id_index : deployment_id_index + 2]

    code = main(args)

    captured = capsys.readouterr()
    assert code == 1
    assert "deployment_id" in captured.err
    assert captured.out == ""


def test_get_byoc_control_panel_state_rejects_invalid_recent_limit(
    monkeypatch,
    capsys,
) -> None:
    monkeypatch.setenv(DEFAULT_SIGNING_SECRET_ENV, SIGNING_SECRET)
    args = _base_args()
    recent_index = args.index("--recent-limit")
    args[recent_index + 1] = "21"

    code = main(args)

    captured = capsys.readouterr()
    assert code == 1
    assert "recent_limit" in captured.err
    assert captured.out == ""


def test_get_byoc_control_panel_state_prints_schema(capsys) -> None:
    code = main(["--schema"])

    captured = capsys.readouterr()
    payload = json.loads(captured.out)
    assert code == 0
    assert "query" in payload
    assert payload["control_panel_state"]["properties"]["schema_version"][
        "const"
    ] == "fyralis.byoc.control_panel_state.v1"
