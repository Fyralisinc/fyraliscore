"""services/ingest/integrations/deel/metrics.py — lightweight in-process counters.
...
def record_request(outcome: str) -> None:
    ...
    key = f"deel.request.{outcome}"
    _counters[key] = _counters.get(key, 0) + 1


def record_provision_outcome(outcome: str) -> None:
    """outcome ∈ {success, no_contracts, error}."""
    key = f"deel.provision.{outcome}"
    _counters[key] = _counters.get(key, 0) + 1
