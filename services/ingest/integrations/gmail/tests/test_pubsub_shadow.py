...
Verifies the shadow block added to `services/app/webhooks/gmail_pubsub.py`:
...
def _patched_oidc(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "services.app.webhooks.gmail_pubsub.verify_pubsub_oidc_token",
...
def _handle_push_returns_tenant(monkeypatch: pytest.MonkeyPatch) -> AsyncMock:
    ...
    monkeypatch.setattr(
        "services.app.webhooks.gmail_pubsub.handle_push",
        mock,
    )
    return mock
...
def _shadow_app(
    ...
    from services.app.webhooks.gmail_pubsub import router as pubsub_router
