"""services/ingest/integrations/notion/webhook.py — Notion webhook ingress (IN-14).
...
a one-time handshake and the steady-state event path:
  ...
  * ``handle_notion_event`` — a verified, tenant-resolved event. Notion
    ...
    services/app/gateway/main.py::_wire_ingestion_data_plane), scoped so the
...
async def _shadow_write_page(
    ...
    *,
    ...
    services/app/gateway/main.py::_wire_ingestion_data_plane). Returns True when
