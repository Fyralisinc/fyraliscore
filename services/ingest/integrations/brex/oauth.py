"""services/ingest/integrations/brex/oauth.py — admin connect wizard (finance).

Brex authenticates with a long-lived Bearer API token (Jira-shaped). This is
...
Flow:
    ...
    POST /integrations/brex/connect/preflight
        body: { api_token, base_url? }
        → BrexClient.list_accounts() to verify the token + enumerate the
          accounts for the selector UI
        → on auth failure: a structured 400 (no secret is stored)

    POST /integrations/brex/connect/finalize
        ...
        → finalize_install(): UPSERT brex_installations + brex_accounts +
          an onboarding_triggers row (source='brex') so the M6 backfill chain
        ...
        → 200 OK with the new brex_installations.id

Backfill (brex_installations) and the live webhook edge
...
def _auth_failure_response(exc: BrexApiError) -> JSONResponse:
    """Map a credential/connectivity failure to a structured 400. The token is
    never echoed back (BrexApiError keeps it off context by design)."""
    unauthorized = getattr(exc, "code", "") == "brex_api_unauthorized"
    return JSONResponse(
        status_code=400,
        content={
            "ok": False,
            "error_code": "brex_auth_failed" if unauthorized else "brex_api_error",
            "message": (
                "Brex rejected the API token. Generate one in Brex "
                "Settings → API tokens and confirm it has read access."
                if unauthorized
                else "Could not reach the Brex API. Check the base_url and "
...
def _normalize_account(a: dict[str, Any]) -> dict[str, Any]:
    """Project a raw Brex account onto the install shape (id/name/kind)."""
    return {
...
async def connect_finalize(request: Request) -> JSONResponse:
    ...
    secret_ref = await store.put(
        api_token, label=f"brex_api_token:{base_url}", tenant_id=tenant_id,
    )
    webhook_secret_ref = None
    if webhook_secret:
        webhook_secret_ref = await store.put(
            webhook_secret, label=f"brex_webhook_secret:{base_url}",
            tenant_id=tenant_id,
        )

    # 3. Install: brex_installations + brex_accounts + onboarding trigger.
