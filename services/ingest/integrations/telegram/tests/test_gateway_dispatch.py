"""Tests for services/ingest/integrations/telegram/gateway/dispatch.py.

The live-dispatch bridge: filtering, the kafka-first cutover (shadow-write to
ingestion.raw.telegram, ingress_kind=gateway), and the inline fallback. The
...
from services.ingest.integrations.telegram.gateway import dispatch as D
...
def _update(mid=1001, *, out=False):
    return {
        ...
        "dialog_id": 42, "dialog_kind": "channel", "dialog_title": "Eng",
    }
...
async def test_inline_when_flag_disabled(monkeypatch):
    ...
    assert called["channel"] == "telegram:message"
