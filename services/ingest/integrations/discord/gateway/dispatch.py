"""services/ingest/integrations/discord/gateway/dispatch.py — dispatch router.
...
from services.ingest.integrations.discord.oauth import short_guild_hash
...
class DispatchDeps:
    ...
    """
    pool: asyncpg.Pool
    tenant_resolver: Any  # services.app.webhooks.tenant_resolver.TenantResolver
    ...
    # silently no-ops.
    s3_raw_client: Any = None    # services.ingest.ingestion.raw_tier.s3.S3Client | None
    kafka_producer: Any = None   # services.ingest.ingestion.kafka.IdempotentProducer | None
    tenant_flags: Any = None     # services.ingest.ingestion.feature_flags.TenantFlags | None
...
async def handle_message_create(message: dict[str, Any], deps: DispatchDeps) -> None:
    ...
    # services/app/webhooks/router.py:741-771) — inline is the source of
