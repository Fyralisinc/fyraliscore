"""services/ingest/integrations/discord/gateway/session_state.py — Discord
   Gateway session state persistence.

Per ingestion LLD §1.5 + M4.2 work order.

Replaces in-memory `session_id` / `last_seq` storage in
[client.py:94](services/ingest/integrations/discord/gateway/client.py#L94) with
...
([services/ingest/ingestion/writers/dlq_writer/dlq_writer.py:345-351](../../ingestion/writers/dlq_writer/dlq_writer.py#L345-L351)).
