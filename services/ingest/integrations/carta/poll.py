"""services/ingest/integrations/carta/poll.py — live POLL dispatch (cap-table).

Carta is POLL-ONLY: there is NO webhook. The live edge re-lists changed
cap-table objects on an interval and dispatches each detected change directly
through the ingestion pipeline — the Telegram-gateway `handle_update` analog,
but driven by a poller instead of a persistent connection.

A Carta poller holds ONE firm's install = ONE tenant's install, so the tenant is
known by construction (carried on `PollDeps`) — there is no per-change tenant
resolution.
...
Flow (parallel to the telegram-gateway cutover):
  ...
  2. Cutover: if `ingestion.kafka_path_enabled` for the tenant (kafka-first
     default), shadow-write the record to `ingestion.raw.carta`
...
authenticated OAuth poll connection itself (as with Telegram's gateway).
"""
...
log = structlog.get_logger("integrations.carta.poll")


CHANNEL = "carta:object"
...
def build_change_record(
    change: dict[str, Any], *, firm_id: str,
) -> dict[str, Any] | None:
...
def _poll_raw(record: dict[str, Any]) -> tuple[bytes, dict[str, Any]]:
    ...
    publishes — so the normalizer feeds `handle_carta_object` identically and
...
async def _attempt_poll_cutover(
    ...
    """Publish the polled record to `ingestion.raw.carta`. Returns True on
    ...
    try:
        await shadow_write_raw(
            ...
            source="carta",  # type: ignore[arg-type]  # wiring widens SourceLiteral
            ingress_kind="poll",
            raw_body=raw_body,
        ...
        log.warning(
            "carta_poll.cutover_failed", error=str(exc)[:200],
        )
        return False
...
async def handle_polled_change(change: dict[str, Any], deps: PollDeps) -> None:
    ...
    if flag_enabled:
        ...
        log.warning("carta_poll.kafka_path_fallback_to_inline")
