...
def _slack_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from services.ingest.integrations.slack import metrics as slack_metrics
    slack_metrics.reset()
...
def _make_gmail_app(pool: asyncpg.Pool, tenant_id: UUID) -> FastAPI:
    ...
    mounted directly in `services/app/gateway/main.py`. The test imports
    the router module and includes it explicitly.
    """
    from services.ingest.integrations.gmail.oauth import (
