"""services/ingest/integrations/fireflies/oauth.py — admin connect wizard.

Fireflies authenticates with a long-lived Bearer API token (Jira/Brex-shaped).
This is the production install surface: real credentials persisted encrypted via
the gateway `secret_store` (only opaque refs reach the install tables). It
mirrors the Brex connect wizard — Bearer-authed, but resolves a single
workspace (no per-account selector; Fireflies has no sharded sub-resource).

Flow:

    POST /integrations/fireflies/connect/preflight
        body: { api_token, base_url? }
        → FirefliesClient.get_workspace() to verify the token + resolve the
          workspace id for the install
        → on auth failure: a structured 400 (no secret is stored)

    POST /integrations/fireflies/connect/finalize
        body: { api_token, base_url?, workspace_id?, webhook_secret? }
        → re-verify creds, resolve the workspace id (probed, or the supplied
          `workspace_id` override)
        → store the API token (+ webhook secret, if given) in the secret store
        → finalize_install(): UPSERT fireflies_installations + an
          onboarding_triggers row (source='fireflies') so the M6 backfill chain
          fires; when a webhook secret is supplied,
          register_webhook_installation() seeds the provider_installations row
          the webhook edge resolves the tenant + HMAC secret from
        → 200 OK with the new fireflies_installations.id

Backfill (fireflies_installations) and the live webhook edge
(provider_installations) are seeded together but stay independent.
"""
from __future__ import annotations

from typing import Any
from uuid import UUID

import asyncpg
import structlog
from fastapi import APIRouter, HTTPException, Request
from fastapi.responses import JSONResponse

from lib.shared.errors import FirefliesApiError
from services.ingest.integrations.fireflies.client import FirefliesClient
from services.ingest.integrations.fireflies.onboarding import (
    finalize_install,
    register_webhook_installation,
)


log = structlog.get_logger("integrations.fireflies.oauth")


# Default Fireflies API host for the connect-wizard UI fallback only (an operator
# may override per-install via the `base_url` field). The canonical default + env
# override live in `lib/integrations/endpoints.py` (`fireflies_api`).
# CONFIRMED (docs.fireflies.ai): the API is GraphQL — a single POST endpoint at
# https://api.fireflies.ai/graphql. Auth is `Authorization: Bearer <api_key>`
# (the key is issued in the Fireflies app's Integrations → Developer settings;
# there is NO OAuth bounce — the connect wizard takes the API key directly).
_DEFAULT_BASE_URL = "https://api.fireflies.ai"


router = APIRouter(prefix="/integrations/fireflies", tags=["fireflies"])


def _tenant_from_request(request: Request) -> UUID:
    auth = getattr(request.state, "auth", None)
    if auth is None or getattr(auth, "tenant_id", None) is None:
        raise HTTPException(status_code=401, detail="unauthenticated")
    tid = auth.tenant_id
    return tid if isinstance(tid, UUID) else UUID(str(tid))


def _pool_from_request(request: Request) -> asyncpg.Pool:
    pool = getattr(request.app.state, "pool", None)
    if pool is None:
        raise HTTPException(status_code=500, detail="database pool unavailable")
    return pool


def _secret_store_from_request(request: Request) -> Any:
    store = getattr(request.app.state, "secret_store", None)
    if store is None:
        raise HTTPException(status_code=500, detail="secret store unavailable")
    return store


def _require_token(body: dict[str, Any]) -> tuple[str, str]:
    api_token = (body.get("api_token") or "").strip()
    base_url = (body.get("base_url") or _DEFAULT_BASE_URL).strip().rstrip("/")
    if not api_token:
        raise HTTPException(status_code=400, detail="api_token is required")
    if not base_url.startswith(("https://", "http://")):
        raise HTTPException(status_code=400, detail="base_url must be a full URL")
    return api_token, base_url


def _auth_failure_response(exc: FirefliesApiError) -> JSONResponse:
    """Map a credential/connectivity failure to a structured 400. The token is
    never echoed back (FirefliesApiError keeps it off context by design)."""
    unauthorized = getattr(exc, "code", "") == "fireflies_api_unauthorized"
    return JSONResponse(
        status_code=400,
        content={
            "ok": False,
            "error_code": "fireflies_auth_failed" if unauthorized else "fireflies_api_error",
            "message": (
                "Fireflies rejected the API token. Generate one in Fireflies "
                "Settings → Developer Settings and confirm it has read access."
                if unauthorized
                else "Could not reach the Fireflies API. Check the base_url and "
                "that the service is reachable."
            ),
            "underlying_error": str(exc)[:300],
        },
    )


def _resolve_workspace(ws: dict[str, Any]) -> tuple[str, str | None]:
    """Project a raw Fireflies workspace onto (workspace_id, workspace_name)."""
    wid = str(ws.get("workspace_id") or ws.get("id") or ws.get("teamId") or "")
    name = ws.get("workspace_name") or ws.get("name")
    return wid, name


@router.post("/connect/preflight")
async def connect_preflight(request: Request) -> JSONResponse:
    """Verify the API token and resolve the workspace for the install."""
    _tenant_from_request(request)  # auth check
    body = await request.json()
    api_token, base_url = _require_token(body)

    client = FirefliesClient(base_url=base_url, api_token=api_token)
    try:
        ws = await client.get_workspace()
    except FirefliesApiError as exc:
        return _auth_failure_response(exc)
    finally:
        await client.aclose()

    workspace_id, workspace_name = _resolve_workspace(ws if isinstance(ws, dict) else {})
    return JSONResponse(content={
        "ok": True,
        "base_url": base_url,
        "workspace_id": workspace_id,
        "workspace_name": workspace_name,
    })


@router.post("/connect/finalize")
async def connect_finalize(request: Request) -> JSONResponse:
    """Persist credentials + install the source.

    Credentials are verified BEFORE any secret is written, so an invalid token
    leaves no `encrypted_secrets` / install rows behind.
    """
    tenant_id = _tenant_from_request(request)
    pool = _pool_from_request(request)
    store = _secret_store_from_request(request)
    body = await request.json()
    api_token, base_url = _require_token(body)

    workspace_override = (body.get("workspace_id") or "").strip() or None
    webhook_secret = (body.get("webhook_secret") or "").strip() or None

    # 1. Verify creds + resolve the workspace — before any write.
    client = FirefliesClient(base_url=base_url, api_token=api_token)
    try:
        ws = await client.get_workspace()
    except FirefliesApiError as exc:
        return _auth_failure_response(exc)
    finally:
        await client.aclose()

    probed_id, workspace_name = _resolve_workspace(ws if isinstance(ws, dict) else {})
    workspace_id = workspace_override or probed_id
    if not workspace_id:
        raise HTTPException(
            status_code=400,
            detail="could not resolve a workspace_id (supply workspace_id explicitly)",
        )

    # 2. Persist secrets encrypted-at-rest; only opaque refs reach the DB.
    secret_ref = await store.put(
        api_token, label=f"fireflies_api_token:{base_url}", tenant_id=tenant_id,
    )
    webhook_secret_ref = None
    if webhook_secret:
        webhook_secret_ref = await store.put(
            webhook_secret, label=f"fireflies_webhook_secret:{base_url}",
            tenant_id=tenant_id,
        )

    # 3. Install: fireflies_installations + onboarding trigger.
    install_id = await finalize_install(
        pool,
        tenant_id=tenant_id,
        base_url=base_url,
        workspace_id=workspace_id,
        workspace_name=workspace_name,
        secret_ref=secret_ref,
        webhook_secret_ref=webhook_secret_ref,
    )

    # 4. Live webhook edge — needs the workspace id (the webhook installation_id)
    #    and a signing secret.
    webhook_registered = False
    if webhook_secret_ref:
        await register_webhook_installation(
            pool,
            tenant_id=tenant_id,
            workspace_id=workspace_id,
            webhook_secret_ref=webhook_secret_ref,
        )
        webhook_registered = True

    log.info(
        "fireflies.connect.finalized",
        installation_id=str(install_id),
        workspace_id=workspace_id,
        webhook_registered=webhook_registered,
    )
    return JSONResponse(content={
        "ok": True,
        "installation_id": str(install_id),
        "workspace_id": workspace_id,
        "webhook_registered": webhook_registered,
    })


__all__ = ["router"]
