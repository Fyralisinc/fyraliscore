"""services/ingest/integrations/figma/metrics.py — lightweight in-process counters.
...
def record_request(outcome: str) -> None:
    ...
    key = f"figma.request.{outcome}"
    _counters[key] = _counters.get(key, 0) + 1


def record_provision_outcome(outcome: str) -> None:
    """outcome ∈ {success, no_files, error}."""
    key = f"figma.provision.{outcome}"
    _counters[key] = _counters.get(key, 0) + 1
