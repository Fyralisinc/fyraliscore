"""services/ingest/integrations/signal/gateway/dispatch.py — live update dispatch.

The bridge between the persistent Signal linked-device session (worker.py) and
the ingestion pipeline. `handle_update` is called for every live message the
session delivers. It is the Telegram-gateway `handle_update` analog, with the same
simplification: a Signal gateway worker holds ONE linked account's session = ONE
tenant's install, so the tenant is known by construction (carried on
`DispatchDeps`) — there is no per-update tenant resolution.

Flow (parallel to the telegram / discord-gateway cutover):
  ...
  2. Cutover: if `ingestion.kafka_path_enabled` for the tenant (kafka-first
     default), shadow-write the record to `ingestion.raw.signal`
  ...
  3. Fallback / inline: otherwise `core.ingest("signal:message", record, …)`,
     then a best-effort M2 shadow audit (when SHADOW_WRITE_ENABLED).

There is NO HMAC signature gate — the trust boundary is the authenticated Signal
linked-device session itself (as with Telegram's / Discord's gateway).
"""
...
log = structlog.get_logger("integrations.signal.gateway.dispatch")
...
def _update_to_record(update: dict[str, Any], deps: DispatchDeps) -> dict[str, Any] | None:
    ...
    `update` shape (produced by the worker's receive handler and by the synthetic
    generator):
        {"event": "new_message",
         "message": {id, date, edit_date, message, out, from_id, …},
         "thread_id": int, "thread_kind": str, "thread_title": str|None}
    """
    if update.get("event") != "new_message":
        return None
    message = update.get("message")
    thread_id = update.get("thread_id")
    if not isinstance(message, dict) or not isinstance(thread_id, int):
        return None
    # Skip our own outgoing messages (the linked account's sends).
    ...
    return build_message_record(
        ...
        thread_id=thread_id,
        thread_kind=update.get("thread_kind") or "direct",
        thread_title=update.get("thread_title"),
    )
...
def _gateway_raw(record: dict[str, Any]) -> tuple[bytes, dict[str, Any]]:
    ...
    publishes — so the normalizer feeds `handle_signal` identically and
...
async def _attempt_gateway_cutover(
    ...
    """Publish the live record to `ingestion.raw.signal`. Returns True on
    ...
    try:
        ...
        log.warning(
            "signal_gateway.cutover_failed", error=str(exc)[:200],
        )
        return False
