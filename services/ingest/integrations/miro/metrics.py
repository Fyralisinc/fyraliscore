"""services/ingest/integrations/miro/metrics.py — lightweight in-process counters.

Mirrors the brex/google_drive metrics shape: a module-level dict bumped by the
...
def record_request(outcome: str) -> None:
    ...
    key = f"miro.request.{outcome}"
    _counters[key] = _counters.get(key, 0) + 1


def record_provision_outcome(outcome: str) -> None:
    """outcome ∈ {success, no_boards, error}."""
    key = f"miro.provision.{outcome}"
    _counters[key] = _counters.get(key, 0) + 1
