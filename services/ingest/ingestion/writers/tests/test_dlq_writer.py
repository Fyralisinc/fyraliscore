...
async def test_dlq_writer_consumes_and_upserts(fresh_db: asyncpg.Pool):
    from services.ingest.ingestion.writers.dlq_writer import (
...
async def test_dlq_writer_handles_concurrent_inserts_via_unique_constraint(
    ...
    from services.ingest.ingestion.writers.dlq_writer import upsert_failure
...
async def test_dlq_writer_continues_on_postgres_error(
    ...
    the consumer.
    """
    from services.ingest.ingestion.writers import dlq_writer as dlq_writer_mod
    from services.ingest.ingestion.writers.dlq_writer import (
        DLQWriterConfig, run_dlq_writer,
    )
    ...
    with KafkaContainer("confluentinc/cp-kafka:7.6.1") as kafka:
        ...
        monkeypatch.setattr(
            "services.ingest.ingestion.writers.dlq_writer.dlq_writer.upsert_failure",
            flaky_upsert,
        )
