...
def _invoice(**over):
    ...
    base.update(over)
    return {"_fyralis_record_type": "invoice", "_fyralis_business_id": _BUSINESS,
            "entity": base}
...
async def test_open_invoice_is_signal_with_synctoken_external_id():
    draft = await handle_ramp_transaction(_invoice(), {})
    ...
    assert draft.external_id == f"ramp:{_BUSINESS}:txn:1037:open"
...
async def test_paid_invoice_is_state_change():
    ...
    draft = await handle_ramp_transaction(base, {})
    assert draft.kind == "state_change"
    assert draft.content["status"] == "paid"
    # state token folds the SyncToken in so each per-edit mutation is distinct.
    assert draft.external_id == f"ramp:{_BUSINESS}:txn:1037:paid.1"
...
async def test_synctoken_bump_produces_distinct_external_id():
    ...
    v0 = await handle_ramp_transaction(_invoice(), {})
    bumped = _invoice()
    bumped["entity"]["SyncToken"] = "1"
    v1 = await handle_ramp_transaction(bumped, {})
    assert v0.external_id != v1.external_id


async def test_bill_record():
    draft = await handle_ramp_transaction({
        "_fyralis_record_type": "bill", "_fyralis_business_id": _BUSINESS,
    ...
    assert draft.external_id == f"ramp:{_BUSINESS}:txn:204:open"
    assert "AWS" in draft.content_text


async def test_payment_record_is_signal():
    draft = await handle_ramp_transaction({
        "_fyralis_record_type": "payment", "_fyralis_business_id": _BUSINESS,
...
async def test_invoice_line_items_and_dimensions_captured():
    ...
    draft = await handle_ramp_transaction(inv, {})
    c = draft.content
