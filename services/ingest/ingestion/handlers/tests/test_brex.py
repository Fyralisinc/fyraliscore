"""Tests for services/ingest/ingestion/handlers/brex.py (finance)."""
from __future__ import annotations

import pytest

from services.ingest.ingestion.handlers import CHANNEL_TRUST_MAP, get_handler
from services.ingest.ingestion.handlers.brex import handle_brex_transaction
...
async def test_handler_registered():
    assert get_handler("brex:transaction") is handle_brex_transaction
    assert CHANNEL_TRUST_MAP["brex:transaction"] == "authoritative"


async def test_transaction_is_signal_with_versioned_external_id():
    draft = await handle_brex_transaction(_txn(), {})
    assert draft.source_channel == "brex:transaction"
    ...
    assert draft.external_id == f"brex:{_ACCT}:txn:t-1001:sent"
...
async def test_failed_transaction_is_state_change():
    ...
    assert draft.external_id == f"brex:{_ACCT}:txn:t-1001:failed"
...
async def test_status_change_produces_distinct_external_id():
    ...
    earlier observation."""
    sent = await handle_brex_transaction(_txn(), {})
    failed = await handle_brex_transaction({
        "_fyralis_record_type": "transaction", "_fyralis_account_id": _ACCT,
        "transaction": {"id": "t-1001", "amount": -5000.0,
                        "counterpartyName": "Acme Cloud", "status": "failed",
                        "createdAt": "2026-05-21T08:00:00.000Z"},
...
async def test_account_snapshot_is_signal_with_balance():
    ...
    assert draft.external_id == f"brex:{_ACCT}:balance:2026-05-31"
...
async def test_webhook_transaction_created():
    ...
    draft = await handle_brex_transaction(payload, {})
    assert draft.content["object_type"] == "transaction"
...
async def test_backfill_and_webhook_dedup_to_same_external_id():
    backfill = await handle_brex_transaction(_txn(), {})
    webhook = await handle_brex_transaction({
...
async def test_details_routing_is_pii_redacted():
    draft = await handle_brex_transaction(_txn(details={
...
async def test_extras_absent_keys_not_emitted():
    ...
    assert "brex_category" not in draft.content
...
async def test_unknown_payload_raises():
    ...
    with pytest.raises(ValidationError):
        await handle_brex_transaction({"foo": "bar"}, {})
