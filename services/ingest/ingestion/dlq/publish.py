"""services/ingest/ingestion/dlq/publish.py — best-effort DLQ publish helpers.

Per M3.1. Used by both the normalizer worker
(`services/ingest/ingestion/normalizer/worker.py`) and the no-op writer
(`services/ingest/ingestion/writers/observation_writer.py`) so the
...
async def publish_dlq(
    *,
    producer: Any,  # services.ingest.ingestion.kafka.IdempotentProducer
