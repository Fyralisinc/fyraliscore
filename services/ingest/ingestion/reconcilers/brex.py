"""services/ingest/ingestion/reconcilers/brex.py — gap detection (finance).
...
idempotent. The probe also serves as the ~45-day token keepalive (Brex
deletes idle tokens).
"""
...
def _get_pool():  # noqa: ANN202
    if _pool_provider is None:
        raise RuntimeError(
            "reconcilers.brex: pool provider not registered. "
...
async def _open_brex_client(install: asyncpg.Record):  # noqa: ANN202
    from services.ingest.ingestion.fetchers.brex import _open_brex_client
    return await _open_brex_client(install)
...
async def reconcile_brex(
    ...
    if new_shards:
        return ReconciliationDecision(
            ...
            message=f"brex reconciler: {len(new_shards)} gap(s).",
        )
    return ReconciliationDecision(has_gaps=False)


RECONCILER_DISPATCH["brex"] = reconcile_brex


__all__ = ["reconcile_brex", "set_pool_provider", "SHARD_KIND_ACCOUNT_TXNS"]
