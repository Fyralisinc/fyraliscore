...
def _get_pool():  # noqa: ANN202
    if _pool_provider is None:
        raise RuntimeError(
            "reconcilers.ramp: pool provider not registered. "
...
async def _open_ramp_client(install: asyncpg.Record):  # noqa: ANN202
    from services.ingest.ingestion.fetchers.ramp import _open_ramp_client
    return await _open_ramp_client(install)
...
async def reconcile_ramp(
    ...
    install = await pool.fetchrow(
        """
        SELECT id, tenant_id, business_id, base_url, secret_ref,
    ...
    if new_shards:
        return ReconciliationDecision(
            ...
            message=f"ramp reconciler: {len(new_shards)} gap(s).",
        )
    return ReconciliationDecision(has_gaps=False)


RECONCILER_DISPATCH["ramp"] = reconcile_ramp


__all__ = ["reconcile_ramp", "set_pool_provider", "SHARD_KIND_ENTITY"]
