"""CLI entry for `python -m services.ingest.ingestion.normalizer`.
...
from services.ingest.ingestion.normalizer.supervisor import (
    SupervisorConfig,
    run_supervisor,
)
from services.ingest.ingestion.normalizer.worker import main as worker_main


def cli() -> None:
    p = argparse.ArgumentParser(prog="services.ingest.ingestion.normalizer")
    p.add_argument(
        "--single-worker",
        action="store_true",
