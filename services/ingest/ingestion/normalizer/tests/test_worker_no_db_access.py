...
def test_worker_import_graph_contains_no_db_modules() -> None:
    ...
    Mechanism: the subprocess `import`s `services.ingest.ingestion.normalizer.worker`
    ...
    script = textwrap.dedent("""
        import sys
        import services.ingest.ingestion.normalizer.worker  # noqa: F401
        violations = sorted(
    ...
    assert violations == [], (
        ...
        "(see services/ingest/ingestion/handlers/gmail.py for the canonical "
        "lazy-import pattern)."
    )
...
def test_supervisor_import_graph_contains_no_db_modules() -> None:
    ...
    script = textwrap.dedent("""
        import sys
        import services.ingest.ingestion.normalizer.supervisor  # noqa: F401
        violations = sorted(
            m for m in sys.modules
            if m == "asyncpg"
            or m.startswith("asyncpg.")
