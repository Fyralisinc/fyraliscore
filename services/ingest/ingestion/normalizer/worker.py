...
async def _normalize_one_with_envelope(
    ...
    *,
    ...
    direct_channel = supplemental_source_channel(envelope.source)
    connector_supported = (
        connector_router is not None and connector_router.supports(envelope.source)
    )
    if not connector_supported and direct_channel is None:
        raise RuntimeError(
            f"source connector is unavailable for {envelope.source!r}"
        )
    channel = direct_channel or f"{envelope.source}:connector"
    ...
    # Stable fleet sources delegate semantics to their connector manifest.
    # The two consolidation carry-forwards retain their existing rich direct
    # semantic handlers until their SourceConnector-v1 ports are admitted.
    ...
    if direct_channel is not None:
        ...
        draft = await get_handler(direct_channel)(payload, headers)
    else:
    ...
    normalized = NormalizedEnvelope(
        ...
        ingress_kind=envelope.ingress_kind,
        tenant_id=envelope.tenant_id,
        connector_installation_id=envelope.connector_installation_id,
        ...
        artifact_descriptors=draft.artifact_descriptors,
