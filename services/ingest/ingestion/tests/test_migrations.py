...
async def test_rls_policy_isolates_by_tenant(rls_app_pool: asyncpg.Pool):
    # Runs as a non-super role (root conftest::rls_app_pool) so RLS is
    # actually enforced. A superuser/BYPASSRLS connection sees through the
    # policy regardless of FORCE, which would make this test vacuous.
