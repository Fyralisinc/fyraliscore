"""Fixtures for services/ingest/ingestion tests.

Wave 2-A chose to place shared fixtures in a helper module
(`services/app/gateway/tests/_shared_fixtures.py` is inlined here for
simplicity — duplicating is safer than cross-conftest imports under
pytest's fixture-resolution rules).

Uses the same per-test-pool + TRUNCATE pattern as services/app/gateway/
tests/conftest.py.
"""
...
async def _run_migrations(conn: asyncpg.Connection) -> None:
    ...
    # after later cutover migrations (for example 0230 dropping legacy
