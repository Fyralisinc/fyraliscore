"""services/ingest/ingestion/workflows/state.py
...
under `services/ingest/ingestion/workflows/` MUST go through these substrate
...
pattern — is:
  ...
  2. Flush the producer; await broker-acks (A6 precedent — same shape
     as `services/ingest/integrations/discord/gateway/_durability.py::pre_save_flush`).
