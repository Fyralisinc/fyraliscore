...
(a) **New-request phase.** Consume `source_onboarding_requested`
    ...
    `gmail_installations`, resolve the source contract's planner and
    call it with `PlannerContext` → `list[Shard]`. INSERT one row per
...
============================================================
  ...
  Rule 5 (no cross-workflow shared state):
    No module-level mutable state in this file. Planner callables are
...
SELECT onboarding_run_id, source, tenant_id, installation_row_id, status
...
INSERT INTO onboarding_shards
    (id, onboarding_run_id, tenant_id, source, shard_kind,
     ...
     recency_score, state, created_at)
VALUES ($1, $2, $3, $4, $5, $6::jsonb, $7, $8, $9, $10, 'pending', now())
"""

_LOAD_SHARD_SQL = """
SELECT id, onboarding_run_id, tenant_id, source, shard_kind, shard_identifier,
       installation_row_id, state
...
async def _load_install(
    ...
    """Load only the contract-bound, exact enabled installation row."""
...
async def _build_source_client(
    ...
    """
    builder = resolve_planner_client_builder(source)
    return await builder(pool, install) if builder is not None else None


async def _insert_shard(
    conn: asyncpg.Connection, *,
    shard_id: UUID, run_id: UUID, tenant_id: UUID, source: str,
    installation_row_id: UUID, shard: Shard,
) -> None:
    ...
    declared_installation = identifier.get("installation_row_id")
    if (
        declared_installation is not None
        and str(declared_installation) != str(installation_row_id)
    ):
        raise ValueError(
            "planner returned a shard bound to a different installation row"
        )
    identifier["installation_row_id"] = str(installation_row_id)
    await conn.execute(
        _INSERT_SHARD_SQL,
...
def _sanitize_connection_error(failure_reason: object) -> str:
    """Make a bounded UI-visible source sync error safe to persist.
    ...
    return safe[:_FIGMA_CONNECTION_ERROR_MAX_CHARS] or "Source sync failed; retry shortly"
...
async def _mark_source_run_failed(
    ...
    *,
    ...
    """Atomically mark a source run failed and invoke its declared hook.
    ...
    async with bind_tenant(conn, tenant_id) as tctx:
        ...
        failure_handler = resolve_onboarding_failure_handler(source)
        if (
            result.endswith(" 1")
            and failure_handler is not None
            ...
            await failure_handler(
                ...
                tenant_id=tenant_id,
                installation_id=installation_row_id,
                failure_reason=_sanitize_connection_error(failure_reason),
            )
...
class SourceOnboarding(LongRunningService):
    ...
    async def _handle_source_requested(
        ...
        try:
            ...
            failure_reason = (
                "Historical source onboarding requires an exact installation "
                "row UUID."
            )
        ...
        if run["tenant_id"] != tenant_id:
            failure_reason = (
                "Source-onboarding signal tenant does not own the source run."
            )
            await _mark_source_run_failed(
                conn,
                run_id=run_id,
            ...
            return []
        if run["installation_row_id"] != installation_row_id:
            failure_reason = (
                "Source-onboarding signal installation does not match the "
                "durable source run."
            )
            await _mark_source_run_failed(
                conn,
        ...
        try:
            planner = resolve_planner(source)
            shards = await planner(ctx)
    ...
    async def _handle_shard_completed(
        ...
        if await _any_shard_failed(conn, run_id=run_id, source=source):
            ...
            await _mark_source_run_failed(
                ...
                installation_row_id=shard["installation_row_id"],
            )
            await self._emit_source_completed(
