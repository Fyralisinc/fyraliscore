...
async def test_retry_with_backoff_on_429_logs_per_attempt(
    ...
    monkeypatch.setattr(
        "services.ingest.ingestion.workflows.retry.asyncio.sleep", _fake_sleep,
    )
    ...
    caplog.set_level(logging.INFO, logger="services.ingest.ingestion.workflows.retry")
...
async def test_retry_indefinitely_on_transient_stops_at_max_elapsed(
    ...
    monkeypatch.setattr(
        "services.ingest.ingestion.workflows.retry.time.monotonic", _monotonic,
    )
    monkeypatch.setattr(
        "services.ingest.ingestion.workflows.retry.asyncio.sleep", _fake_sleep,
    )
