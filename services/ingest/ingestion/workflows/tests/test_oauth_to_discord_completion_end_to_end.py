...
from services.ingest.ingestion.fetchers import discord as fdc
from services.ingest.ingestion.reconcilers import discord as rdc
from services.ingest.ingestion.workflows import source_onboarding as so_mod
...
async def _run_pipeline(*, fresh_db, helpers_dir, helper_name, label):
    ...
    for role, svc, var, extras in [
        ...
        if role == "orch":
            cmd = [sys.executable, "-m", f"services.ingest.ingestion.workflows.{svc}"]
        else:
            cmd = [sys.executable, "-c",
                   bootstrap.format(svc_main=f"services.ingest.ingestion.workflows.{svc}")]
