...
The five-rule analyzer enforced over `services/ingest/ingestion/workflows/`.
...
SCOPE
============================================================
The analyzer runs over `services/ingest/ingestion/workflows/*.py` (excluding
...
WORKFLOWS_DIR = REPO_ROOT / "services" / "ingest" / "ingestion" / "workflows"
...
RETROACTIVE_PRECEDENTS = [
    REPO_ROOT / "services" / "ingest" / "ingestion" / "recovery"
                  / "embedding_backlog" / "embedding_backlog.py",
    REPO_ROOT / "services" / "ingest" / "ingestion" / "feature_flags"
                  / "circuit_breaker.py",
]
...
def _rule_2_state_in_postgres(
    ...
    for node in tree.body:
        if isinstance(node, ast.ImportFrom) and node.module:
            if node.module == "services.ingest.ingestion.workflows.state":
    ...
    if not found_state_import:
        return [Violation(
            ...
            detail=(
                ...
                "services.ingest.ingestion.workflows.state. The substrate "
                    ...
                    if (node.module, alias.name) in _FORBIDDEN_QUEUE_PRIMITIVES:
                        violations.append(Violation(
                            ...
                            detail=(
                                ...
                                f"use services.ingest.ingestion.workflows.signals "
                ...
                if (recv.id, node.func.attr) in _FORBIDDEN_QUEUE_PRIMITIVES:
                    violations.append(Violation(
                        ...
                        detail=(
                            ...
                            f"Use services.ingest.ingestion.workflows.signals for "
...
def _retroactive_rules(path: pathlib.Path) -> list[Violation]:
    """Subset of rules applicable to files outside services/ingest/ingestion/workflows/.
...
def test_pattern_alignment_passes_for_workflows_dir() -> None:
    """Every concrete file under services/ingest/ingestion/workflows/ MUST
    ...
    if all_violations:
        ...
        raise AssertionError(
            ...
            f"services/ingest/ingestion/workflows/:\n{formatted}\n\n"
