"""Tests for services/ingest/ingestion/fetchers/brex.py (finance)."""
from __future__ import annotations

import pytest

from services.ingest.ingestion.fetchers import brex as brex_fetcher
from services.ingest.ingestion.fetchers.brex import (
    BrexCursor,
...
class _FakeClient:
    """Implements the BrexClient read surface the fetcher uses."""
...
class _FakeInst:
    _d = {"base_url": "https://api.brex.com/api/v1", "tenant_id": None,
          "secret_ref": None}
...
def _wire(monkeypatch, client):
    ...
    monkeypatch.setattr(brex_fetcher, "_open_brex_client", _open)
...
async def test_full_backfill_emits_snapshot_plus_transactions(monkeypatch):
    ...
    cur = BrexCursor.model_validate(res.next_cursor)
...
async def test_missing_account_id_is_noop(monkeypatch):
    client = _FakeClient({}, [], [])
    _wire(monkeypatch, client)
    res = await fetch_page_brex(_FakeInst(), {"shard_kind": SHARD_KIND_ACCOUNT_TXNS}, None)
    assert res.records == []
    assert res.end_of_data is True
