...
class _FakeClient:
    """Implements the RampClient.query surface the fetcher uses."""
...
class _FakeInst:
    _d = {"business_id": _BUSINESS, "base_url": "https://x", "tenant_id": None,
          "secret_ref": None}
...
async def test_incremental_warm_start_adds_where_filter(monkeypatch):
    ...
    shard = {"shard_kind": SHARD_KIND_ENTITY, "entity_type": "Invoice",
             "business_id": _BUSINESS, "updated_cursor": "2026-05-09T00:00:00-08:00"}
    res = await fetch_page_ramp(_FakeInst(), shard, None)
