...
from services.ingest.source_contract.models import Certification, IngressRoute
...
def test_normalization_and_live_contracts_are_explicit_for_every_source() -> None:
    for source in SOURCE_DEFINITIONS:
        ...
        for transport, ack, delivery, binding in source.live_contracts():
...
def test_onboarding_contracts_cover_all_sources_and_routes() -> None:
    ...
    for source in SOURCE_DEFINITIONS:
        ...
        assert paths
        assert all(
            path.startswith(f"/integrations/{source.source_id}/")
            for path in paths
        )
...
def test_all_68_ingress_routes_are_owned_by_the_source_contract() -> None:
    routes = {
...
def test_effective_request_policy_is_owned_by_the_exact_source_operation() -> None:
    ...
    assert (
        request_policy_for_operation("google", "messages.get")
        == policy
    )
...
def test_transport_operation_catalog_is_derived_from_source_contracts() -> None:
    ...
    assert all(
        source.provider_transport_enforced for source in SOURCE_DEFINITIONS
    )
...
def test_finance_testing_surface_is_a_source_capability() -> None:
    ...
    ) == ("mercury", "quickbooks", "brex", "ramp", "gusto", "deel")
