"""MockCartaClient — Carta query surface used by IN-CARTA backfill.
...
what the backfill/poll fetcher + reconciler call:
  ...
  - firm_info()  -> connectivity probe (the reconciler's gap reference call)

Pagination mirrors the real client's offset semantics exactly so the fetcher's
cursor loop behaves identically:
  - `start_position` is 1-based (CARTA STARTPOSITION); the slice is
...
class MockCartaClient(_MockBase):
    ...
    def __init__(
        ...
        *,
        ...
        self._firm_id = str(fixture.get("firm_id", ""))
    ...
    async def query(
        ...
        *,
        ...
        # CARTA STARTPOSITION is 1-based.
    ...
    async def firm_info(self) -> dict[str, Any]:
        """`GET /v1/firms/{firm}/firminfo/{firm}` analogue — a connectivity probe
        used by the reconciler. Returns a minimal FirmInfo body."""
        self._check_fault()
        return {
            "FirmInfo": {
                "Id": self._firm_id or "1",
                "FirmName": f"Synthetic Firm {self._firm_id}",
            },
        }
