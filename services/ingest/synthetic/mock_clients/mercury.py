"""MockMercuryClient — Mercury banking API surface used by the finance backfill.

In-process replacement for `MercuryClient` (services/ingest/integrations/mercury/
client.py). Implements the read methods the planner-less Mercury chain calls:
...
class MockMercuryClient(_MockBase):
    """Stateful in-process replacement for `MercuryClient`.
    ...
    async def get_account(self, account_id: str) -> dict[str, Any]:
        ...
        if not isinstance(acct, dict):
            raise MercuryApiError(
                f"MockMercuryClient: account {account_id!r} not found",
    ...
    async def has_transactions_since(
        ...
        `since`. Mirrors reconcilers/mercury.py's `list_transactions(limit=1,
    ...
    # ---- Fault raisers (surface the real MercuryApiError + codes) ----
    def _raise_rate_limit(self) -> NoReturn:
        raise MercuryApiError(
            "MockMercuryClient: rate limit (X2 fault)",
    ...
    def _raise_5xx(self) -> NoReturn:
        raise MercuryApiError(
            "MockMercuryClient: 503 (X2 fault)",
    ...
    def _raise_auth_error(self) -> NoReturn:
        raise MercuryApiError(
            "MockMercuryClient: 401 token rejected (X2 fault)",
    ...
    def _raise_transient(self) -> NoReturn:
        raise MercuryApiError(
            "MockMercuryClient: transient transport error (X2 fault)",
            code="mercury_api_error",
