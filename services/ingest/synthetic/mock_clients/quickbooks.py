"""MockQuickBooksClient — QuickBooks Online query surface used by IN-FIN backfill.

In-process replacement for `QuickBooksClient` at the `_open_quickbooks_client`
fetcher seam (services/ingest/ingestion/fetchers/quickbooks.py). Implements ONLY
...
cursor loop behaves identically:
  - `start_position` is 1-based (QBO STARTPOSITION); the slice is
...
four raisers surface `QuickBooksApiError` with the same stable `code`s the real
...
class MockQuickBooksClient(_MockBase):
    """Stateful in-process replacement for `QuickBooksClient`.

    `fixture` shape (per `make_quickbooks`):
        {
          "realm_id": "9341452000000001",
          "page_size": 100,
          "entities": {
            "Invoice": [ {<full QBO entity>}, ... ],
            "Bill":    [ ... ],
            "BillPayment": [ ... ],
            "Payment": [ ... ],
          },
    ...
    def __init__(
        ...
        *,
        ...
        self._realm_id = str(fixture.get("realm_id", ""))
    ...
    async def company_info(self) -> dict[str, Any]:
        """`GET /v3/company/{realm}/companyinfo/{realm}` analogue — a connectivity
        probe used by the reconciler. Returns a minimal CompanyInfo body."""
        self._check_fault()
        return {
            "CompanyInfo": {
                "Id": self._realm_id or "1",
                "CompanyName": f"Synthetic Co {self._realm_id}",
            },
        }
    ...
    def _raise_rate_limit(self) -> NoReturn:
        raise QuickBooksApiError(
            "MockQuickBooksClient: rate limit 429 (X2 fault)",
    ...
    def _raise_5xx(self) -> NoReturn:
        raise QuickBooksApiError(
            "MockQuickBooksClient: 503 (X2 fault)",
    ...
    def _raise_auth_error(self) -> NoReturn:
        raise QuickBooksApiError(
            "MockQuickBooksClient: 401 access token rejected (X2 fault)",
    ...
    def _raise_transient(self) -> NoReturn:
        raise QuickBooksApiError(
            "MockQuickBooksClient: transient transport error (X2 fault)",
            code="quickbooks_api_error",
            context={"error_type": "TransportError"},
        )


__all__ = ["MockQuickBooksClient"]
