"""services/ingest/synthetic/mock_servers/mercury.py — local Mercury banking REST mock.

A real, threaded HTTP server that mimics the Mercury v1 endpoints the ingestion
path touches, so a sandbox can drive the REAL MercuryClient + fetcher +
reconciler against it with no Mercury credentials:
...
where each transaction is a raw Mercury transaction object. The mock does not
synthesize them so the sandbox controls exactly what lands.

The client is pointed here via the spammer single-host base
(`SYNTHETIC_SOURCE_API_BASE=<base>` -> `<base>/mercury`); the handler matches on
...
MercuryFixtures = dict[str, dict[str, Any]]

_ACCOUNT_TXNS_RE = re.compile(r"/account/([^/]+)/transactions$")
_ACCOUNT_RE = re.compile(r"/account/([^/]+)$")


def _make_handler(fixtures: MercuryFixtures, hits: dict[str, int]):
...
def start_mock_mercury(
    fixtures: MercuryFixtures, *, host: str = "127.0.0.1", port: int = 0,
) -> tuple[ThreadingHTTPServer, str]:
    """Start the mock on a background daemon thread.

    Returns `(server, base_url)`. Point the client at `base_url` via
    `SYNTHETIC_SOURCE_API_BASE` (spammer mode, served under `/mercury`). Call
...
__all__ = ["MercuryFixtures", "start_mock_mercury"]
