...
A real, threaded HTTP server that mimics the Gusto v3 query endpoint
the ingestion path touches, so a sandbox can drive the REAL GustoClient +
fetcher + reconciler against it with no Intuit credentials:

  GET /v3/company/{company}/query?query=<SQL>&minorversion=75
  ...
  GET /v3/company/{company}/companyinfo/{company}
      Connectivity probe.

Fixtures: {entity_name: {"rows": [...], "delta": [...]}} where each row is a raw
GUSTO entity object (Id, SyncToken, MetaData.LastUpdatedTime, ...). `delta` is the
...
`/gusto`); the handler matches the `/v3/company/.../query` path SUFFIX so
...
GustoFixtures = dict[str, dict[str, list[dict[str, Any]]]]
