"""services/ingest/synthetic/mock_servers/quickbooks.py — local QuickBooks Online mock.

A real, threaded HTTP server that mimics the QuickBooks Online v3 query endpoint
the ingestion path touches, so a sandbox can drive the REAL QuickBooksClient +
fetcher + reconciler against it with no Intuit credentials:

  GET /v3/company/{realm}/query?query=<SQL>&minorversion=75
...
QBO entity object (Id, SyncToken, MetaData.LastUpdatedTime, ...). `delta` is the
...
`/quickbooks`); the handler matches the `/v3/company/.../query` path SUFFIX so
...
QuickBooksFixtures = dict[str, dict[str, list[dict[str, Any]]]]
...
def _make_handler(fixtures: QuickBooksFixtures, hits: dict[str, int]):
...
def start_mock_quickbooks(
    fixtures: QuickBooksFixtures, *, host: str = "127.0.0.1", port: int = 0,
...
__all__ = ["QuickBooksFixtures", "start_mock_quickbooks"]
