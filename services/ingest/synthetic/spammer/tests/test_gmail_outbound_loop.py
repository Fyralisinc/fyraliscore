...
runs (`python -m services.ingest.synthetic.spammer.server`). What's being proven is
...
async def _real_gmail_client(app):
    ...
    from services.ingest.integrations.gmail import dwd as dwd_mod
    from services.ingest.integrations.gmail.client import GmailClient, GoogleHttpClient
...
async def test_real_gmail_client_hits_spammer(monkeypatch):
    from services.ingest.integrations.gmail.client import GMAIL_METADATA_SCOPE
