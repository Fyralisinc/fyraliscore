    ...
    def _conversations_list(
        ...
        for prefix in ("lab-slack-user::", "spam-slack-user::"):
            if token.startswith(prefix) and types & {"im", "mpim"}:
                ...
                key = f"{team}::{user}"
                conversations = list(
                    (state.get("direct_messages") or {}).get(key, [])
                )
                filtered = [
                    conv
    ...
    def _conversations_history(
        ...
        if oldest is not None:
            messages = [
                ...
                if float(item.get("ts", "0")) > float(oldest)
            ]
...
class GithubAdapter:
    ...
    async def handle(self, request: ProviderRequest) -> ProviderResponse:
        ...
        if route_id in {"github.repo_issues", "github.repo_pulls"}:
            event_type = "issues" if route_id.endswith("issues") else "pull_requests"
    ...
    def _repositories(self, request: ProviderRequest) -> ProviderResponse:
        ...
        per_page = max(
            1, min(100, int(request.query_one("per_page", "30") or "30"))
        )
    ...
    def _repo_events(
        ...
        etag = f'W/"{full_name}:{event_type}:v{len(events)}"'
        ...
        return ProviderResponse.json(events[start:end], headers=headers)
...
class GmailAdapter:
    ...
    def _directory(request: ProviderRequest) -> ProviderResponse:
        ...
        if route_id == "gmail.directory_org_units":
            return ProviderResponse.json(
                {
                    "organizationUnits": copy.deepcopy(
                        directory.get("org_units") or []
                    )
                }
            )
        if route_id == "gmail.directory_group_members":
            ...
            rows = list(
                (directory.get("group_members") or {}).get(group_key, [])
            )
            return ProviderResponse.json(
        ...
        if route_id == "gmail.directory_users":
            ...
            if query.startswith("orgUnitPath="):
                ...
                rows = [
                    row
                    for row in rows
                    if str(row.get("orgUnitPath", "")) == org_unit
                ]
            return ProviderResponse.json(
                _google_directory_page(request, "users", rows)
            )
    ...
    for fixture in fixtures:
        ...
        for email, raw_mailbox in mailbox_items:
            ...
            mailbox["current_history_id"] = str(
                mailbox.get("current_history_id")
                or mailbox.get("history_id")
                or "1000"
            )
...
def _seed_discord(fixtures: list[Mapping[str, Any]]) -> Mapping[str, Any]:
    ...
    for fixture in fixtures:
        ...
        for channel in fixture.get("channels", []):
            state["guilds"][guild_id]["channels"].append(
                {
                    "id": channel["id"],
                    "name": channel.get("name"),
