...
Cloned from the QuickBooks archetype. `make_ramp(business_id=..., entities=[...],
rows_per_entity=N)` produces a deterministic per-entity-type fixture shaped to
feed `MockRampClient`. The mock paginates each entity list by the archetype's
offset cursor (`STARTPOSITION` / `start_position`) and the fetcher drives one
`ramp_entity` shard per entity type. (Entity taxonomy is the archetype default —
...
(business_id, entity_type, idx). Re-running with the same args yields byte-identical
output.
"""
...
def make_ramp(
    *,
    business_id: str = "9341452000000001",
    ...
    Args:
      business_id: RAMP company business id (stamped into refs + returned at top level).
      ...
      Fixture dict consumed by `MockRampClient(fixture=...)`:
        {
          ...
          "page_size": 100,
          "entities": {
            "Invoice": [ {<full RAMP entity>}, ... ],   # ordered oldest-first
            "Bill":    [ ... ],
            ...
          },
        }
