"""Brex accounts/transactions fixture generator (finance).

`make_brex(accounts=N, transactions_per_account=M, ...)` produces a
deterministic Brex install fixture shaped to feed `MockBrexClient`. It
...
Fixture shape (consumed by `MockBrexClient(fixture=...)`):
    {
      "accounts": {
        "<account_id>": {
...
The fetcher (services/ingest/ingestion/fetchers/brex.py) emits, per account:
...
# Default per-account kinds (cycled across accounts). Mirrors Brex's
...
def make_brex(
    *,
    ...
    """Build a deterministic Brex install fixture.
    ...
    Args:
      ...
      base_iso: Anchor timestamp (2026-01); txns are spaced backwards from it
        so the list is newest-first (Brex's ordering).
      ...
      seed: Optional namespacing salt mixed into the synthetic `account_id`
        ...
        `brex:{account_id}:txn:…`). Default "" preserves the original ids;
        ...
        production where each tenant's Brex org has distinct account ids.
    ...
    Returns:
      Fixture dict consumable by `MockBrexClient(fixture=...)`.
    """
    ...
    for a in range(accounts):
        ...
        _acct_parts = ("brex-account", a) if not seed else ("brex-account", seed, a)
...
def _txn(account_id: str, idx: int, base_date: str) -> dict[str, Any]:
    """One deterministic Brex transaction, newest-first by `idx`.

    idx=0 is the newest; later indices are older — matching Brex's
    ...
    return {
        ...
        "brexCategory": "Vendors" if amount < 0 else "Revenue",
        "postedAt": iso,
        "createdAt": iso,
        "details": {
