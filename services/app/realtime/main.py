"""services/app/realtime/main.py — WebSocket sub-router + app wiring.
...
def _validate_token_lazy():
    """Lazy import to avoid the `services.app.gateway.__init__` → `main`
...
def _realtime_deps(ws_or_request) -> RealtimeDeps:
    ...
    if deps is None:
        raise RuntimeError(
            ...
            "services.app.realtime.configure_realtime(app, pool) at startup"
        )
    return deps
