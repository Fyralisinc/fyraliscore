...
Verifies the shadow-write block added to `services/app/webhooks/router.py`:
...
from services.app.webhooks.tenant_resolver import Resolved
...
def _stub_ingest(monkeypatch: pytest.MonkeyPatch):
    """Replace `services.app.webhooks.router.ingest` with an AsyncMock so
    ...
    monkeypatch.setattr(
        "services.app.webhooks.router.ingest",
        mock_ingest,
    )
    return mock_ingest
