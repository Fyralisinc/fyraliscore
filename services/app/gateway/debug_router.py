"""services/app/gateway/debug_router.py — read-only inspector endpoints.
...
def _jsonify(row: asyncpg.Record | None) -> dict | None:
    ...
    for k, v in dict(row).items():
        if isinstance(v, (datetime,)):
            ...
            k.endswith("_json")
            or k in ("payload", "proposition", "content", "ops_applied")
        ):
            try:
                out[k] = json.loads(v)
            except Exception:  # noqa: BLE001
...
def build_debug_router() -> APIRouter:
    ...
    router.add_api_route(
        "/think-quality",
        get_think_quality_report,
        methods=["GET"],
    )
    router.add_api_route(
        "/think-quality/cases",
        get_think_quality_cases,
        methods=["GET"],
    )
    router.add_api_route(
        "/sage-health",
    ...
    router.add_api_route(
        ...
        list_relationship_ontology_proposals,
        methods=["GET"],
    )
    router.add_api_route(
        "/relationship-ontology-proposals/{proposal_id}/review",
        review_relationship_ontology_proposal,
        methods=["POST"],
    )
    router.add_api_route(
        "/models",
...
async def list_think_runs(
    ...
    if trigger_kind:
        ...
        where.append(f"trigger_kind = ${len(args)}")
    async with pool.acquire() as conn:
...
async def get_think_quality_report(
    ...
    tid = _resolve_tenant(req)
    pool = await _pool_from_request(req)
    from services.reasoning.think.quality_report import build_think_quality_report
...
async def list_models(
    ...
    if kind:
        args.append(kind)
        where.append(f"proposition_kind = ${len(args)}")
...
async def list_renders(
    ...
    args: list[Any] = [tid]
    if render_kind:
        args.append(render_kind)
        where.append(f"render_kind = ${len(args)}")
    async with pool.acquire() as conn:
        rows = await conn.fetch(
