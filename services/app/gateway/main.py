...
def build_app(
    *,
    ...
    async def _lifespan(app_: FastAPI) -> AsyncIterator[None]:
        ...
        runtime_owns_embedder = False

        async with contextlib.AsyncExitStack() as stack:
            stack.callback(startup_status.mark_stopped)
            github_cleanup_client = None
            if bool(getattr(app_.state, "gateway_owns_github_client", False)):
                existing_github_client = getattr(app_.state, "github_client", None)
                if existing_github_client is not None:
                    github_cleanup_client = existing_github_client
            ...
            if deps is None:
                ...
                try:
                    ...
                    runtime_actor_repo = completed_deps.actor_repo
                    runtime_alias_repo = completed_deps.alias_repo
                    runtime_embedder = completed_deps.embedder
                    runtime_rate_limiter = completed_deps.rate_limiter
                    runtime_owns_embedder = completed_deps.owns_embedder
                    ...
                    raise
                app_.state.gateway_owns_embedder = runtime_owns_embedder
                if runtime_owns_embedder:
                    stack.push_async_callback(
                ...
                runtime_owns_embedder = bool(
                    getattr(app_.state, "gateway_owns_embedder", False)
                )
                if runtime_owns_embedder:
                    stack.push_async_callback(
                        _close_owned_embedder_for_lifespan,
            ...
            try:
                wire_integration_runtime_state(app_, runtime_pool)
            ...
            try:
                ...
                if github_wiring.owns_client and github_client is not github_cleanup_client:
                    github_cleanup_client = github_client
                    stack.push_async_callback(
                        close_github_gateway_state,
                        app_,
                        client=github_client,
                    )
                startup_status.ok("github_gateway_state", required=True)
            except Exception as exc:  # noqa: BLE001
                startup_status.failed_component(
            ...
            if settings.ceo_view_enabled:
                previous_ceo_view = getattr(app_.state, "ceo_view", None)
                try:
                    await _await_startup(
                        "ceo_view",
                        configure_ceo_view(app_, pool=runtime_pool),
            ...
            if data_plane_wired:
                ...
                if "ingestion_data_plane" not in startup_status.components:
                    startup_status.disabled(
                        "ingestion_data_plane",
                        required=settings.require_ingestion_data_plane,
