"""
services/domain/models/tests/test_repo.py — integration tests for ModelsRepo.

BUILD-PLAN §0.5: every test runs against live Postgres. We follow the
observations-service pattern (see services/domain/observations/tests/conftest.py):
...
async def test_topology_insert_failure_does_not_poison_model_insert(
    ...
    from services.domain.models import repo as repo_mod
...
async def test_hourly_decay_helper_runs(
    ...
    """services.domain.models.decay.hourly_decay helper runs and returns rowcount."""
...
async def test_add_note_and_list_notes(
    ...
    from services.domain.models.status_notes import add_note, list_notes
