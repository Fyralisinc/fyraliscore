"""
services/domain/models/tests/conftest.py — shared fixtures for Models tests.
...
Fixture architecture mirrors services/domain/observations/tests/conftest.py:
...
def pytest_collection_modifyitems(config, items):  # noqa: D401
    ...
    for item in items:
        if "services/domain/models/tests/" in str(item.fspath):
...
async def tenant(tx_conn: asyncpg.Connection) -> uuid.UUID:
    tenant_id = uuid7()
...
@pytest_asyncio.fixture
async def other_tenant(tx_conn: asyncpg.Connection) -> uuid.UUID:
    tenant_id = uuid7()
    await tx_conn.execute(
