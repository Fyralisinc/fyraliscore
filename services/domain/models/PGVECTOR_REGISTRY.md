# `services.domain.models` — pgvector pool-shared registry contract

## Why this document exists

`services.reasoning.retrieval.pathways:pathway_b_semantic` (the cosine-similarity
...
```python
# services/reasoning/retrieval/pathways.py:763
...
The check goes through `services.domain.models.repo.PGVECTOR_REGISTERED_POOL_IDS`
...
| Site                                              | Method  |
|---------------------------------------------------|---------|
| `services/app/gateway/db_bootstrap.py`                | manual init body |
| `tests/synthesis_harness/__main__.py`             | `init=pgvector_pool_init` |
| `services/domain/models/repo.py:_ensure_vector_codec`    | per-conn lazy fallback |
| `services/reasoning/retrieval/pathways.py:_conn_has_vector_codec` | reader |
