"""services/domain/bridge/tests/conftest.py — per-test asyncpg pool with
JSONB + vector codecs installed, migrations applied + TRUNCATE between
tests. Mirrors services/domain/resources/tests/conftest.py.

We intentionally duplicate the conftest pattern instead of importing
`services.domain.resources.tests.conftest` so the two test packages can run
