"""services/integrations/mercury/onboarding.py — install + provision (finance).
...
from services.integrations.mercury import metrics
...
async def finalize_install(
    ...
    *,
    ...
    the mercury_installations id. Idempotent on (tenant_id, base_url) and per
    (install, account_id).
    """
    ...
    async with tenant_transaction(tenant_id, pool=pool) as tctx:
        install_id = await tctx.fetchval(
            ...
            ON CONFLICT (tenant_id, base_url) DO UPDATE
                SET secret_ref = COALESCE(EXCLUDED.secret_ref, mercury_installations.secret_ref),
