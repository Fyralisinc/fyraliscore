"""services/github_intel/worker.py — ordered state-advancement + enrichment worker.
...
from services.github_intel.code_client import blast_radius_for, extract_changed_paths
from services.github_intel.config import GITHUB_INTEL_LLM_ENABLED, MAX_BLAST_HOPS
from services.github_intel.enrichment import assemble_intelligence, _related_entities
from services.github_intel.state_store import apply_event
...
async def _process_item(
    ...
    if llm_enabled and not fsm.is_obvious(ev):
        try:
            from services.github_intel.reasoner import llm_causal
