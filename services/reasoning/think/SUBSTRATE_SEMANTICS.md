...
- Code: `services/reasoning/think/reconciler.py`, `services/reasoning/think/reconciliation_prompt.py` (PR 4).
...
- Code: `services/reasoning/think/hierarchy.py`, `services/reasoning/retrieval/primary.py` extensions, `services/jobs/hierarchy_audit.py` (PR 5).
...
- Schema migration: adds enum + columns + indexes; rollback documented (PR 3). Explicit design-review gate; design doc is `services/reasoning/think/PR3_DESIGN.md`.
- Code: `services/reasoning/think/precondition_resolver.py` (new), `services/reasoning/think/cascade.py` extension, recommendation-feed filter (PR 3).
...
- Code: `services/reasoning/think/prompt.py` (calibration anchor), `services/reasoning/think/reconciler.py` and `services/reasoning/think/applier.py` (`commitment_merge_confidence` branch). No schema change; prompt + reconciler changes only.
...
- Code: [services/reasoning/think/audit.py](audit.py); emission points in [services/domain/models/repo.py](../models/repo.py) (`insert`, `archive`, `bulk_confidence_update`) and [services/reasoning/think/applier.py](applier.py) `_apply_claim_op` (`field_update` and `reconciliation_merge` paths).
- Public API: `emit_audit_event`, `get_audit_chain`, `find_re_assertable_event`, `model_state_snapshot`, `emit_reconciliation_merge_audit` — all in [services/reasoning/think/audit.py](audit.py).
