...
`services.domain.models.repo.bulk_confidence_update` machinery (the
...
* Retrieval is unchanged: reconciler reads from existing
  `services.domain.models.repo.search_by_embedding` and the GIN
...
Counters (added to `services.reasoning.think.observability.METRICS`):
...
1. Migration `db/migrations/00XX_reconciliation_events.sql` — new
   ...
   T3.
2. `services/reasoning/think/reconciler.py` — pure logic + config + the
   four-signal match function.
3. Wiring in `services/reasoning/think/applier.py` to call the reconciler
...
6. README at `services/reasoning/think/RECONCILIATION_README.md`
