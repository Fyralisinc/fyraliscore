"""services/reasoning/sage/inquiry_traces/emitter.py — Phase 1 trace emission helpers.
...
Design constraints (Phase 1 wiring agent):
  ...
  * **Tenant/session aware via ContextVar.** No new function-signature
    changes in `services/reasoning/think/*` or
    `services/platform/execution/inquiry.py`. Producers set the
    context, consumers read it.
...
from services.reasoning.sage.inquiry_traces.repo import (
    OmittedEvidenceRepo,
    OutcomeEventsRepo,
    RetrievalPlansRepo,
)
