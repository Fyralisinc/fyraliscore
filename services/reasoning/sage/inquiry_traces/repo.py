"""services/reasoning/sage/inquiry_traces/repo.py — Phase 1 trace repos.

Three repos, one per gap-filler table from migration 0084:
...
Style cribbed from services/domain/models/repo.py and
services/product/forecasts/repo.py: raw asyncpg, structlog, uuid7()
ids, JSONB columns serialized via `json.dumps` and cast `::jsonb` in
the SQL.
"""
...
from services.reasoning.sage.inquiry_traces.types import (
    OMISSION_REASONS,
    OUTCOME_EVENT_TYPES,
    OmittedEvidenceRow,
    OutcomeEventRow,
