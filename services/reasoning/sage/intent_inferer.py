"""services.reasoning.sage.intent_inferer — Phase 3: Retrieval Intent Inferer.

Spec: fyralis-sage-synthesis-self-evolution.md §7.3 (Stage B) and Phase 3
(§1499-1548). Pure-Python, rule-based v1. No DB, no LLM.

Turns ``StructuredCues`` (produced by ``services.reasoning.sage.cue_extractor``)
...
if TYPE_CHECKING:
    ...
    # services/reasoning/sage/cue_extractor.py. It may or may not exist yet —
    # this module only needs duck-typed access to the documented
    # StructuredCues fields, so we keep the runtime import lazy.
    from services.reasoning.sage.cue_extractor import StructuredCues  # noqa: F401
