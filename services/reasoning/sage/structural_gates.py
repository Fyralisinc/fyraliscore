"""services.reasoning.sage.structural_gates — Phase 6: Heuristic Structural Gate Scorer.
...
Upstream produces:
  ...
  * ``RetrievalIntent.intent`` (a question primitive string — see
    ``services.reasoning.sage.intent_inferer.IntentPrimitive``), and a coarse
  ...
  * A candidate graph edge with type (``edge_kind`` in the registry —
    ...
    migration 0085), per-edge ``EdgeStructuralFeatures``, trust tier
...
_PRIMITIVE_ALIASES: dict[str, str] = {
    ...
    # IntentPrimitive (services.reasoning.sage.intent_inferer) → coarse label.
