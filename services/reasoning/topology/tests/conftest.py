"""
services/reasoning/topology/tests/conftest.py — fixtures for topology
integration tests (S2).

Mirrors services/domain/models/tests/conftest.py: per-test asyncpg pool,
...
def _hash_embedding(text: str, dim: int = 768) -> list[float]:
    ...
    services/domain/models/tests/conftest.py — same text → same vector."""
    import hashlib
    import math
    import random
    seed = int.from_bytes(
