"""services/product/demo/router.py — FastAPI APIRouter for every demo endpoint.

Mounted by services/app/gateway/main.py via `_register_demo_routes(app)`.
...
from services.product.demo.repo import (
    get_demo_session,
    list_demo_configs,
    touch_demo_session,
)
from services.product.demo.sessions import end_session, reset_session, start_session
from services.product.demo.simulator import inject_signal, list_suggested_signals
from services.product.demo.sse import stream_for_actor
...
async def get_session_info(session_id: str, request: Request) -> JSONResponse:
    ...
    if sess is None or sess.tenant_id != auth.tenant_id:
        return JSONResponse({"error": "not_found"}, status_code=404)
    from services.product.demo.repo import get_demo_config_by_id
...
async def suggested_signals(request: Request) -> JSONResponse:
    ...
    from services.product.demo.repo import get_active_session_for_tenant, get_demo_config_by_id
...
async def inject(request: Request) -> JSONResponse:
    ...
    from services.product.demo.repo import get_active_session_for_tenant
