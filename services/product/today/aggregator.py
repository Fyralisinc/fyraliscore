"""services/product/today/aggregator.py — derive the Today payload.

The Today UI consumes a single `TodayResponse` shape (see
ui/src/api/today-types.ts). Backend builds that shape by:

  1. Listing active recommendations for the actor (services.product.recommendations.repo)
...
def _derive_severity(view: RecommendationView) -> str:
    ...
    # Normalized regime — impact * confidence. Bands per spec §12.
    score = impact * conf
    if score >= 0.80:
        return "critical"
    if score >= 0.55:
...
def _derive_probe_chips(view: RecommendationView) -> list[dict[str, str]]:
    ...
    The handler in services.product.conversations resolves these ids back to a
