"""services/product/greeting — Agent-GRT: Company OS CEO view pre-compute layer.
...
Module map (COMPANY-OS-UI-BUILD-PLAN.md §3):
  ...
  rendering_adapter.py
                 HTTP adapter into services/product/rendering/ with a
                 deterministic mock for pre-integration development.

Contracts: CONTRACTS.md §1.1 (HTTP shape), §1.4 (WS message shapes),
§2.1 (calls into services/product/rendering/), §2.3 (SubstrateSnapshot), §3
(cache DDL).
"""

from services.product.greeting.cache import (
    CachedContent,
    ViewCeoCacheRepo,
    CACHE_KEYS,
)
...
from services.product.greeting.stream import ViewCeoStreamManager
