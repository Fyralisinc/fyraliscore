...
from services.recommendations.handlers import ratify_hypothesis
...
async def _apply_insert_op(
    ...
    *,
    ...
    tested in services/models/tests and services/think/tests."""
    mid = uuid7()
    embedding = [0.0] * 768
    embedding[0] = 1.0
    await pool.execute(
...
async def _trigger_context_for_row(
    ...
    if isinstance(region_spec, dict):
        ...
        ctx_region = None
    # seed_signature mirrors what services/think/worker.py builds in
