...
-- (Constitution §II.5). The re-added checks are NOT VALID so this
-- historical migration can be safely replayed on a database that already
-- contains rows for sources introduced by later migrations; the latest
-- source-registry migration validates the final superset.
--
...
ALTER TABLE source_onboarding_runs
    ...
    CHECK (source IN ('slack', 'github', 'discord', 'gmail', 'notion')) NOT VALID;
