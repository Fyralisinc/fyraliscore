...
| BYOC local contracts and preflight | Do the BYOC data-plane, permissions/IAM, AWS IaC, Terraform scaffold, product-health automation/install rehearsal, bootstrap bundle, bootstrap dry-run, agent, evidence, aggregate preflight, and clean customer-pilot package rehearsal contracts still pass without cloud credentials, child report details, command output, or customer data leaving the data plane? |
