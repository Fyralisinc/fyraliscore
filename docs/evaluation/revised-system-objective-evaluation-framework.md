...
| Entity grounding, alias learning and relation topology | Opportunity/request/set/assessment/admission/work fates, trace integrity, context-sufficiency admission fencing, exact predecessor/successor grounding, authorized alias learning, four-entity recurrence at `60/60`, matched safety controls, candidate-memory lift and evaluator-owned topology metrics. The positive DB vertical populates five canonical links, two belief Models, two safe no-admissions and one exact-lineage relation. Adversarial v2 rejects four harmful writes, exercises a two-hop chain and exposes immediate correction propagation. The governing extraction population is the audited disjoint broad-v4 holdout; older v3 workstream F1 `0.5` is retained as historical falsifying evidence, not the current headline | Strong bounded extraction plus focused positive and adversarial downstream paths are proven. The extraction holdout does not itself prove canonical linking, while the DB populations do not establish open-world, load, drift or company-scale graph quality. Weak per-type slices and exact denominators remain explicit |
...
```mermaid
flowchart TB
    REG["Suite and metric registries"]
    MAN["Sealed experiment manifest"]
    BASE["Baseline and ablation registry"]
    ORCH["Experiment orchestrator"]
    WORLD["Causal company-world compiler"]
    TRUTH["Evaluator-only hidden truth"]
    ACTOR["Actors and communication engine"]
    SENSOR["Independent source and sensor renderer"]
    CONVTRUTH["Conversational truth compiler"]
    AUTH["Intent and authority oracle"]
    FAULT["Fault and adversary injector"]
    BRIDGE["Fyralis production-shaped test adapter"]
    SYS["Fyralis under test"]
    ACTION["World action and transition engine"]
    FORK["Counterfactual fork manager"]
    TRACE["Trace collector and semantic snapshots"]
    EVAL["Evaluator library"]
    STAT["Statistical analysis"]
    REPORT["System-state report compiler"]
    HIST["Regression and proof history"]
...
```mermaid
flowchart LR
    E["Immutable source events and versions"]
    A["Authority and cutoff gate"]
    S["Permitted source structure and actors"]
    L["Local discourse cues and provisional referents"]
    H["Temporary overlapping conversation hypotheses"]
    P["Context probes and candidate selection"]
    V["Operational sufficiency verdict"]
    C["Interpretation snapshot with embedded selected hash and dependencies"]
    D["Context-conditioned discourse quotation and coreference"]
    M["Mention type and local-role extraction"]
    R["Canonical entity resolution"]
    U["Insufficient or alternate hypotheses"]

    E --> A --> S --> L --> H --> P --> V --> C --> D --> M --> R
    L --> U
    H --> U
    P --> U
    V --> U
...
flowchart LR
    S["Source signal"]
    N["Authorized evidence neighborhood"]
    M["Mention detection"]
    T["Separate type assessment and local roles"]
    Q["Candidate request digest and required lanes"]
    C["One immutable candidate set or terminal fate"]
    A["Evidence-relative resolution assessment"]
    G["Separately justified registry mutation"]
    D["Consumer grounding admission"]
    U["Unresolved distribution review or abstention"]
    X["Consequential company-model use"]
...
flowchart TB
    E0[E0 Contracts]
    E1[E1 Causal worlds]
    E2[E2 Actors and sensors]
    E3[E3 Semantic kernel tests]
    E3S[E3S Source semantics]
    E3C[E3C Conversation reconstruction]
    E3E[E3E Entity grounding]
    E3D[E3D Protocol durability repair]
    E3I[E3I Intent attention Concern]
    E3M[E3M Learned state governance]
    E4[E4 Traces denominators artifacts]
    E5[E5 Brain and graph metrics]
    E5R[E5R Representation utility]
    E6[E6 Context product and human]
    E6V[E6V Operational viability]
    E7[E7 Agency delivery and learning]
    E8[E8 Adversarial protocol and chaos]
    E9[E9 Baselines and ablations]
    E10[E10 Statistics and reports]
    E11[E11 Orchestration]
    E12[E12 Architecture evidence registry]
...
### Current executable entity-physics checkpoint
...
| Audited broad holdout v4 (pre-fix runtime) | 40 disjoint signals in four genuine ten-signal batches; 69 gold, 67 predictions and 66 exact spans; P/R/F1 `0.985075/0.956522/0.970588`; type accuracy `1.0`; 20/20 negatives clean; workstream `6/6`; Slack/email/Jira F1 `0.976744/0.962963/0.974359`; report SHA-256 `f67eb4f…64a245` | Governing bounded broad-extraction evidence. Its receipt lacks a pre-call runtime-source digest, which is an explicit auditability gap. Fixes derived from its misses have unit proof only and must not be credited as fresh post-fix generalization. It does not itself prove canonical linking |
| Current-runtime one-shot holdout v5 | Precommitted in `e6d4b4cd`; 24 organization/entity/time/text-disjoint normalized signals in three genuine eight-signal batches; 35 gold, 35 predictions, 34 exact, all 35 overlap; exact F1 `0.971429`, mean boundary IoU `0.985714`, type `1.0`, 12/12 hard negatives clean, 37/37 raw proposals with terminal fates. Slack `17/17`, project `6/6`, system `6/6`; report SHA-256 `030346c5…854040` | Governing current-runtime generalization evidence for literal spans, role types, same-batch semantic isolation and Slack cross-signal abstention. Pre-call receipt `dd3f07eb…55b6ea` binds the exact corpus, six runtime-source digests, prompt, commit and zero-retry provider config before the single execution. One frozen person gold span omitted the attached title: runtime `Engineer Pavel Ito` follows the bound prompt while gold says `Pavel Ito`; immutable person exact F1 is therefore `0.8` (4/5), type `1.0`, mean IoU `0.9`. No rerun or post-result corpus edit occurred. Canonical alias linking, implicit resolution, connectors, open world and company scale remain unproven |
...
| Objective entity readiness v5 | SHA-binds the historical immutable populations, focused positive/adversarial DB verticals and audited broad-v4 extraction as distinct evidence. Broad-v4 scores `0.990196`; the composed readiness is clear. Artifact `/tmp/objective_entity_evidence_v5.json`, file SHA-256 `83d6a51f…24b2128`, composition SHA-256 `46bd421e…2593af` | Governing bounded entity state, not a release certificate. Historical v3 errors remain history rather than current quality, while v4's missing pre-call runtime-source digest, weak per-type denominators, open-world breadth and company-scale linking remain explicit |
| Objective entity readiness v6 | SHA-binds every v5 population plus the current-runtime report, immutable pre-call receipt and completed execution receipt. Current component score `0.992857`; entity artifact `/tmp/objective_entity_evidence_v6.json`, file SHA-256 `a0f9dc80…f79281`, composition SHA-256 `98fdb546…745f1` | Governing bounded current-runtime entity state, not a release certificate. It closes v4's runtime-source/currentness audit gap while preserving historical v3/v4 outcomes and the frozen person-title disagreement. Canonical/implicit identity, open-world breadth and company-scale linking remain explicit |
...
| Full company simulation | The immutable 45-batch, 1,125-signal run started with zero semantic Models and processed only genuine 25-signal T1 batches. The current entity-v6, synthesis-aware evaluator rerender is `/private/tmp/fyralis-authoritative-45-be401f25/autonomous-learning-cold-start-45-be401f25/authoritative_evaluation_postfix_entity_v6/large_company_simulation_evaluation.json` (SHA-256 `08969dca…302a5`): `not_credible`, score `0.8883`, coverage `1.0`, two company-physics hard failures and 75 proof gaps | All eight bounded learning components are visible at `1.0`; entity evidence observes 22/22 continuous measurements and scores `0.994513`. The recovered Think attempt remains reliability degradation rather than terminal workload loss; the two historical company-physics incidents remain noncompensatory. This is an evaluator-only rerender; the simulation was not rerun, and no second large run is authorized |
...
eligible populations are `unknown`, never implicit passes. Historical composer
v2 (`9bcfde1e`) bound v3, the positive vertical and adversarial v2. Composer v5
additionally required the immutable broad-v4 population. Governing composer v6
also requires the exact-runtime one-shot v5 report and both receipts, and keeps
extraction, canonical linking and downstream topology as separate claims. Its
remaining person-title annotation, per-type breadth, open-world, topology and
semantic-impact limits remain declared rather than averaged away.
...
score. The current entity-v6, synthesis-aware rerender of the sole 45-batch artifact is
`not_credible`, score `0.8883`, evidence coverage `1.0`, with two
noncompensatory company-physics hard failures: 10,325 mention opportunities
without detection fates and 50 resolver-owned identity-registry mutations. The
historical Think failure is now classified as recovered operational degradation,
not terminal workload loss. This is an evaluator-only rerender, not a simulation
rerun, and no second large run is authorized.
