...
| Strict single-Model synthesis | The active v7 holdout's collective-facet judge unions facets across multiple same-tenant Models, so it measures cross-batch evidence accumulation and availability | Strict synthesis remains pending a separate frozen `single-model-synthesis-evaluation-v1` population. For every thesis, exactly one persisted learned-arm Model must contain every required facet and cite eligible prior-Model lineage; the frozen arm must have no such Model. Facets distributed across multiple Models score zero |
...
| Objective company-learning composition | SHA-bound composer keeps seven mandatory component populations independent, fail-closes missing or digest-invalid components and separates observed from coverage-adjusted quality. The joined-runtime vertical and matched feedback-quality proof are independently required; SAGE salience cannot substitute for later-conclusion quality | Current v7 composition at `/private/tmp/objective_company_learning_evidence_v7.json` observes all seven components; coverage, observed score and adjusted score are `1.0`, verdict `meets_bounded_policy`, no below-policy component or blocker. Strict single-Model synthesis is not yet an accepted eighth component. This is a bounded pass with explicit non-open-world, non-customer, no-connector and unbounded-recovery gaps, not release proof |
...
| Full company simulation | The immutable 45-batch, 1,125-signal run started with zero semantic Models and processed only genuine 25-signal T1 batches. The current assurance-backed evaluator rerender is `/tmp/fyralis-authoritative-45-be401f25/autonomous-learning-cold-start-45-be401f25/authoritative_evaluation_postfix_joined_v2/large_company_simulation_evaluation.json`: `not_credible`, score `0.8859`, coverage `1.0`, with the same three historical hard failures | New bounded component evidence is visible, but historical incidents are not rewritten. The simulation itself was not rerun, and no second large run is authorized for this milestone |
