...
The active v7 holdout's collective-facet judge unions facets across multiple
same-tenant Models. Its correct capability label is **cross-batch evidence
accumulation and availability**. It does not prove that Think synthesized one
complete hidden-pattern Model. Actual synthesis requires a separate
`single-model-synthesis-evaluation-v1` population: for every thesis, exactly one
persisted learned-arm Model must contain every required facet and cite prior
Model IDs that belong to the arm's eligible lineage; the frozen arm must have no
such Model. Facets distributed across multiple Models score zero synthesis
recovery even when their tenant-level union is complete.
...
| Objective company-learning composition | SHA-bound composer keeps five component populations independent, fail-closes missing components and separates observed from coverage-adjusted quality | Current v3 composition observes all five components; coverage, observed score and adjusted score are `1.0`, verdict `meets_bounded_policy`, no below-policy component or blocker. This is a bounded pass with explicit non-open-world, non-customer, no-connector and unbounded-recovery gaps, not release proof |
...
| Full company simulation | immutable authoritative 45-batch run remains pre-fix `not_credible`; evaluator-only v3 entity rerender scores `0.8859`, coverage `1.0`, with three hard failures and 75 proof gaps | New bounded component evidence is visible, but historical incidents are not rewritten and no second large run occurred |
