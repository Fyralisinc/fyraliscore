...
This file contains five sequenced Claude Code prompts implementing the v1 substrate decisions from `services/reasoning/think/SUBSTRATE_SEMANTICS.md`. Each PR is run in its own focused Claude Code session. PRs must be merged in order. Do not parallelize.
...
1. Read `services/reasoning/think/SUBSTRATE_SEMANTICS.md` fully. Internalize the five decisions and their interaction map.
...
Produce `services/reasoning/think/V1_BASELINE.md` with sections for each decision and a summary at the top. Each section should be 1-2 paragraphs of concrete findings — not abstractions, but specific code references and current behavior.
...
Implement audit chain preservation per `services/reasoning/think/SUBSTRATE_SEMANTICS.md` Section Q5. Every state change on a Model creates a structured audit entry. Reversal-of-reversal sequences preserve all three events distinctly. Reconciliation merges union both source Models' chains.

## Pre-work

Read `services/reasoning/think/V1_BASELINE.md` Q5 section to confirm current state. The implementation plan depends on what already exists.
...
- `services/reasoning/think/audit.py` — new module for audit chain logic
- `services/domain/models/repo.py` — emit audit events on state changes
...
- `services/reasoning/think/SUBSTRATE_SEMANTICS.md` — update with implementation references
...
2. Every state change in `services/domain/models/repo.py` emits an audit event.
...
Read `services/reasoning/think/V1_BASELINE.md` Q4 section to confirm current prompt state and reconciliation behavior.
...
- `services/reasoning/think/prompt.py` (or equivalent) — update extraction prompt
- `services/reasoning/think/reconciler.py` — add commitment_merge_confidence function and branch
- `services/reasoning/think/applier.py` — call the right merge confidence function based on Node type
...
Read `services/reasoning/think/V1_BASELINE.md` Q3 section.

## Design review gate

Before implementing, produce `services/reasoning/think/PR3_DESIGN.md` answering:
...
- `services/domain/models/repo.py` — Commitment creation handles precondition; emit state-change audit events
- `services/reasoning/think/cascade.py` — extension for precondition satisfaction
- `services/reasoning/think/precondition_resolver.py` — new module for resolving each precondition form
- `services/reasoning/think/applier.py` — handle `state` field in claim_ops
- `services/reasoning/think/validator.py` — validate precondition forms
...
Read `services/reasoning/think/V1_BASELINE.md` Q1 section.

## Design review gate

Before implementing, produce `services/reasoning/think/PR4_DESIGN.md` answering:
...
- `services/reasoning/think/reconciler.py` — three-tier logic
- `services/reasoning/think/reconciliation_prompt.py` — new module for second-pass prompt
- Schema migration for `reconciliation_decisions` table
- `services/reasoning/think/reconciler_cache.py` — cache lookup and invalidation
- `services/reasoning/think/audit.py` — audit logging extensions for reconciliation decisions
- `tests/synthesis_harness/cases_reconciliation_q1.py` — new adversarial scenarios
- `services/reasoning/think/SUBSTRATE_SEMANTICS.md` — update with Q1 implementation reference
...
Read `services/reasoning/think/V1_BASELINE.md` Q2 section.

## Design review gate

This PR's design is the hardest. Before implementing, produce `services/reasoning/think/PR5_DESIGN.md` answering:
...
- `services/reasoning/think/hierarchy.py` — new module
- `services/reasoning/think/extractor.py` — emit relationships
- `services/reasoning/retrieval/primary.py` — Pathway A walk
- `services/reasoning/think/reconciler.py` — second-pass extension
- `services/reasoning/think/precondition_resolver.py` — hierarchy walk
