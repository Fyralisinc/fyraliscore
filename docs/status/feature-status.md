...
unintended. **131 findings total: 21 high (4 resolved — cutover circuit
breaker, Google Calendar install, Google Drive install, Jira install), 46 medium (7 resolved — Kafka full-pipeline persistence,
...
webhook verification metrics, and the Mercury/QuickBooks production install
flow), 64 low** — the high/medium are below; low
