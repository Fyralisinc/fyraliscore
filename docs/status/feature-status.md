...
breaker), 46 medium (4 resolved — Kafka full-pipeline persistence,
`google_drive` async embedding, onboarding progress events, and the
`feels_onboarded_monitor` service), 64 low** — the high/medium are below; low
