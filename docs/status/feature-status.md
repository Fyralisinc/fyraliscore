...
| Provider-operation API quota coordination | Provider clients charge actual calls through shared transport | **✅ Resolved (2026-07-25).** Each client declares operation-specific quota scopes and weighted costs. `RedisQuotaCoordinator` charges them atomically through the generic Lua `RateLimiter`, shares provider cooldowns across replicas, and returns durable `RetryLater` for long waits. The obsolete `FetchRateLimiter`/`BUCKET_DEFAULTS` page gate was removed from `shard_fetch`, so workflows neither fabricate budgets nor sleep while holding work. | ✅ resolved |
