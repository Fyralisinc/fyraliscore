...
| `github_intel` + `code_intel` *(extracted)* | GitHub intelligence + code graph/search — **extracted to `Fyralisinc/github-intel`**, no longer in core. | — | `external` | Returns as the first external interface (see Interfaces & Extensions / ADR-0004). |
