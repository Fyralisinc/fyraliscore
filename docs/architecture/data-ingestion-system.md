...
**Editable diagram:** [`fyralis-data-ingestion-3-planes-9-layers.drawio`](../../fyralis-data-ingestion-3-planes-9-layers.drawio) — open with diagrams.net/draw.io. The embedded Mermaid views below remain convenient for rendered documentation.
...
| Identity and Knowledge | 6. Entity resolution | Immutable complete or partial identity snapshot |
| Identity and Knowledge | 7. Claims, provenance, and access | Evidence-bound semantic claims and policies |
...
#### Consolidation carry-forward adapters

Instagram Messaging and Facebook Pages were completed directly on the previous `consolidation/final` lineage after the 26-source stable connector fleet was defined. Their provider-specific gateway, semantic handler, history planner, fetcher, reconciler, and onboarding behavior is retained in this consolidation so no implemented capability is discarded.

They are deliberately represented as **supplemental ingestion adapters**, not as stable SourceConnector-v1 candidates. They share the same tenant-scoped raw object, Kafka, normalization-envelope, immutable evidence, identity, and episode pipeline, while their existing rich semantic handlers perform normalization. The source catalog exposes a separate supplemental inventory so raw-envelope and topic validation includes both lanes without falsely passing these adapters through connector conformance or release admission. Their eventual connector-contract ports must preserve the current provider-specific identity, conversation, pagination, gap-detection, and webhook semantics before this exception is removed.
...
The normalizer fetches the raw object and invokes the bound connector's identity and normalization capabilities. For the two explicit consolidation carry-forwards, it invokes the retained provider-specific semantic handler instead. Both paths emit `NormalizedEnvelope` records containing `ObservationDraft` fields plus complete raw lineage.
...
### 5.1 Subsystem 7 — Provenance, access, and claims
...
### 5.2 Subsystem 6 — Entity resolution
...
| Area | Current state |
| --- | --- |
| Connector contract, installations, acquisition, raw and normalized lanes | Implemented for 26 admitted SourceConnector-v1 families plus two explicit supplemental carry-forward adapters; all 28 use the shared evidence data plane. |
...
| Source/evidence/identity/episode schema | `db/migrations/0227` through `db/migrations/0240` |
