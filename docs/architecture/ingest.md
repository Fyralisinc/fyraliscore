...
| Google DWD connect | `services/ingest/integrations/{gmail,google_calendar}/oauth.py` | `POST /integrations/{gmail,google_calendar}/connect/{preflight,finalize}` — first-party Domain-Wide-Delegation connect wizard (no OAuth bounce): `preflight` enumerates the Workspace domain for the selector UI (or returns the exact client_id + scopes to grant if the DWD grant is missing); `finalize` resolves the inclusion_spec → mailbox/calendar emails and writes the install + per-resource rows + an `onboarding_triggers` row in one transaction so the M6 backfill chain fires. Both mount in the gateway behind the `GMAIL_SERVICE_ACCOUNT_JSON*` gate (Calendar reuses Gmail's service account). Gmail additionally provisions Pub/Sub watches out-of-band; Calendar is poll-only (no async provisioning). |
