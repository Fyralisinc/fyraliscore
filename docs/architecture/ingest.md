...
| Provider-operation quota coordinator | `services/ingest/ingestion/rate_limit/{client,provider_transport}.py`, `services/ingest/integrations/provider_transport_runtime.py` | Provider clients declare quota scopes and costs for each **actual outbound operation**. `RedisQuotaCoordinator` atomically charges those scopes through the generic Lua `RateLimiter` and propagates provider cooldowns. `shard_fetch` does not guess a fixed page budget or sleep while holding workflow work; a long cooldown becomes durable `RetryLater`. |
