...
| BYOC customer-pilot package | `services/platform/runtime/byoc_customer_pilot_package.py` | Offline backend/core package builder; writes sanitized handoff artifacts and a digest-only manifest under a repo-local output directory, marking cloud/control-plane work manual until real reports are supplied. |
