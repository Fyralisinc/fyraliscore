...
| Candidate and relationship intelligence | `relationship_candidates`, `model_edges`; legacy `model_neighborhoods`, `model_neighborhood_membership`, `topology_events` | `relationship_candidates` is the single pre-truth proposal lifecycle. Active topology is an upstream pattern-discovery origin that writes topology-discovered proposals into that lifecycle with `candidate_lifecycle` metadata. Typed edges store accepted pairwise meaning. Legacy accepted-memory topology tables remain for compatibility and map history. |
| Execution inquiry | `inquiry_sessions`, `inquiry_question_runs`, `inquiry_evidence_items` | Durable adaptive-inquiry sessions, question paths, context packets, and evidence reservoir cards linked to questions, hypotheses, retrieval paths, and sufficiency verdicts. The retired `signal_routing_decisions` table was dropped by migration `0127`; routing helpers are not an active ingest ledger. |
...
9. A `think_trigger_queue` row is written unless the observation was deduped or trigger enqueueing was disabled.
10. Deterministic execution routing helpers still exist under [services/platform/execution/routing.py](services/platform/execution/routing.py), but they are not wired into active ingest.
11. Routing-decision persistence is retired: migration `0127` dropped `signal_routing_decisions`, and active ingest does not write a routing ledger.
...
| Topology discovery | [services/reasoning/topology/field.py](services/reasoning/topology/field.py), [services/reasoning/topology/eval_harness.py](services/reasoning/topology/eval_harness.py), [services/workers/topology_sweeper](services/workers/topology_sweeper), `relationship_candidates` | Upstream pattern discovery for the relationship-candidate lifecycle. Each new/changed Model is converted into an impact signature over flows, pressures, surfaces, stakes, time shape, evidence, and action/falsifier surface. The field searches bounded semantic, surface, consequence, and evidence pools, scores consequence interactions, persists only high-yield edge/situation/edge-type proposals as `relationship_candidates`, and marks them with `candidate_lifecycle.origin_stage = pattern_discovery`. The sweeper periodically revisits high-activation Models so older memory can connect to newer memory without a full all-pairs recompute. |
| Accepted-memory topology (legacy schema) | `model_neighborhoods`, `model_neighborhood_membership`, `topology_events` | Retired graph-derived positional embeddings, neighborhoods, topology events, and projection helpers. The Python repos/workers for this engine have been removed; remaining database tables stay for compatibility/history and current map/Today/decision-delta surfaces that read historical rows. `topo_dirty_queue` was dropped by migration `0127`. |
| Relationship candidates | [services/reasoning/relationships](services/reasoning/relationships), `relationship_candidates` | The single ranked pre-acceptance lifecycle for new edges, situation Models, or edge-type gaps. Topology-discovered rows are not a second candidate class; they are relationship candidates whose lifecycle metadata records `origin = latent_topology`, `origin_stage = pattern_discovery`, and `discovery_pattern_kind`. Candidate adjudication marks each row accepted, rejected, retired, or needs-review after T4 Think interprets the proposal. Causal candidates require an explicit mechanism summary and can carry intervention surface, expected delay, and confounders in metadata before promotion. |
