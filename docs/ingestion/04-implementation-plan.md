...
##### M6.2b — Reconciler (next work-unit; deferred)

- `services/ingestion/workflows/reconciler.py` — reconciliation orchestration: triggers on `source_onboarding_completed`; queries per-source authoritative count APIs (M6.3-M6.6 implement those); detects coverage gaps >0.1%; INSERTs new `onboarding_shards` rows with `state='reconciliation_resharded'` + `parent_shard_id`; re-enters the fetch loop via `shard_fetch_requested` emits.
- Tests: `test_reconciler_detects_gap_and_reshares_shard`.
- Inheritance from M6.2a: A15 schema (no new migration); A16 transactional patterns (likely CLAIM-VIA-UPDATE + N1 composition); shared-inbox pattern.
