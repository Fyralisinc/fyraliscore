...
`services/product/greeting/scheduler.py:173-191`: each gateway replica with `GATEWAY_START_GRT_SCHEDULER=1` (the docker-compose default) spawns its own refresh / TOD-boundary / post-commit-listener loops. The `_started` flag is in-process only.
...
`services/reasoning/think/reason.py:182-195, 584`: the entire think transaction (with the region lock) is held while `llm_reason()` runs, often 5–30s.

`services/domain/models/repo.py:444-448, 507, 635`: `ModelsRepo.insert()` opens a transaction, then calls `OllamaClient.embed()` for the natural-language form — another 100–500ms+ of network I/O inside the lock.
...
`services/reasoning/think/applier.py:102-124`: SELECT-then-INSERT against `applied_triggers` (`trigger_id` PK). Two concurrent workers processing the same `trigger_id` collide on the PRIMARY KEY. The `IntegrityError` is unhandled, the trigger is marked failed, retries hit the same race.
...
`services/reasoning/think/worker.py:321-331`: rows are leased via `locked_by = worker_id, locked_at = now()`. There is no expiry filter on the poll query and no in-flight heartbeat. A crashed or hung worker leaves the row locked forever; new workers don't see it.
...
`services/app/gateway/main.py:559-566`: the session-mint endpoint trusts `tenant_id` from JSON body without comparing to the header. Combined with P1-5 below, this expands the bootstrap-secret blast radius.
...
`services/product/greeting/rendering_adapter.py:342-351, 469-490`: `_post()` makes a single httpx call with a 30s timeout, no retries. On failure the caller silently falls back to `_synthesize_placeholder_reasoning`, hiding the failure.

`services/product/query/adapters.py:187`: `httpx.AsyncClient` is recreated per call (no pooling).
...
`services/app/gateway/main.py:134-142`: the middleware does `log.error(...)` then `raise`. If structlog or the renderer throws (e.g., circular ref in payload), the original exception is lost and the request returns an opaque 500 with no trace.
...
`services/reasoning/think/worker.py:556-601` writes `model_reeval_dead_letter`; same for post-commit dead-letter. No admin endpoint, no metric, no alert.
...
`services/app/gateway/main.py:1933` — gating by env name leaks raw prompts and substrate in any environment a developer labels "staging".
...
`services/app/gateway/main.py:521-536` — if `COMPANY_OS_ENV != prod` and `AUTH_BOOTSTRAP_SECRET` blank, anyone can mint a session.
...
`services/app/gateway/debug_router.py:350` and `services/app/gateway/main.py:1660` — table/column names interpolated into SQL strings. Currently safe (allowlists exist) but fragile.
...
`services/product/rendering/core.py:705-714` — `loop.create_task(_do_insert())` with no `add_done_callback`. Cost rows lost on transient DB failure; billing/metrics blind.
...
- `services/product/query/core.py:227-229`: card resolver bare `except Exception` → returns wrong-context answers
- `services/product/query/core.py:360-364`: prefetch deserialization bare `except` → cache poisoning invisible
- `services/reasoning/retrieval/assembler.py:478-481`: bridge revenue lookup → silent `None`, rendering proceeds
...
`lib/shared/db.py:66-71` and `services/app/gateway/db_bootstrap.py:94-100` — many distinct query shapes; cache thrashes under load.
...
`services/app/gateway/main.py:591` — full request body is read before size check, allowing memory DoS.
...
`services/reasoning/think/worker.py:366-390` — the documented bound is "≤ 5 generations" but actual reject fires at 5 (so 0..4 succeed).
...
### P2-22: `services/ingest/synthetic/` has zero tests despite being a substrate-injection path

**Priority:** P2 · **Effort:** M

~288 LoC, no `services/ingest/synthetic/tests/`. Bypasses dedup and T1 enqueue.
...
### P2-23: Strategy mocks in `services/product/query/tests/test_core.py` skip real ranking

**Priority:** P2 · **Effort:** M

`services/product/query/tests/test_core.py:39-48` — replaces every retrieval strategy with `FakeStrategy`. The dispatcher is tested; the actual scoring (RRF, relevance) is not.
...
`services/product/rendering/tests/test_api.py:62-66` — `ScriptedProvider` returns prebaked strings. We never assert that real Anthropic/OpenAI/DeepSeek responses parse correctly.
...
`services/app/gateway/main.py:386` — dispatcher tasks started from lifespan don't carry `request_id`/`tenant_id` contextvars.
...
`services/reasoning/think/reason.py:410-633` — debug rows extend critical-section duration even when capture is for observability only.
...
| P3-20 | `services/product/query/core.py` and `services/product/recommendations/repo.py` lack type hints on private helpers | S |
...
- `services/ingest/synthetic/` (~288 LoC, substrate injection path)
- `services/workers/maintenance/scheduler.py` (orchestration; only indirect coverage)
- `services/ingest/ingestion/handlers/slack.py` signature verification (covered only as integration side-effect)
