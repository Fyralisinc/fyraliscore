...
> source dispatch and source-local implementations. Migration `0230` completes
...
- the stateless normalizer bypasses durable artifact admission and durable
  authority;
- migration 0227 and database/Kafka/S3 integration gates were not executed;
...
| Common installation/authority migration | △ | Migration 0227 provides a common header and seeds Slack, Notion, and WhatsApp only. The remaining sources and provider lifecycle owners are not migrated. |
...
12. **Migration version mismatch.** migration 0227 seeds Slack/Notion
...
WhatsApp default to connector mode even though migration 0227, real
...
8. **Run release-environment gates.** Apply/rollback migration 0227 on a
